...
// Returns true if all occurrences of .toUtf8().constData() in `src` are safe.
// Scans: is a stored QByteArray variable used nearby? Is it in a return/send call?
static bool checkUtf8ConstDataSafety(const QString& src, QStringList* failures) {
    const QStringList lines = src.split('\n');
    bool allSafe = true;
    // Match varName.toUtf8().constData() pattern
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        // Check preceding 2 lines for stored QByteArray
        bool safe = false;
...
public:
    AutoCompletionSafetyTest() : QObject() {}
...
public:
    FunctionCallTipSafetyTest() : QObject() {}
    void run() {
        QDir d(__FILE__);
        d.cdUp(); d.cdUp();
...
public:
    ScintillaEditorSafetyTest() : QObject() {}
    void run() {
        QDir d(__FILE__);
        d.cdUp(); d.cdUp();
...
public:
    ...
    void run() {
        ...
        // Widen the window enough to catch all four guards (empty-path + container guards).
        ...
        // The empty-path guard must show a user-visible error (QMessageBox warning).
        QVERIFY2(openFnBody.contains("QMessageBox::warning"),
                 "openFileInTab() must show QMessageBox::warning on validation failure");
        // Guard: path must be validated as an existing regular file before editor construction.
        // This prevents ScintillaEditor creation for directories, broken symlinks, etc.
    ...
    void run() {
        ...
        if (showCount > 0) {
            ...
            QFAIL(qPrintable("showMainWindow() called directly in main() — "
                              "duplicate causes double app.exec() crash:\n"
                              + occurrences.join("\n")));
        }
    }
};
...
public:
    FindReplaceDialogSafetyTest() : QObject() {}
    void run() {
        QDir d(__FILE__);
        d.cdUp(); d.cdUp();
...
public:
    FunctionListPanelSafetyTest() : QObject() {}
    void run() {
        QDir d(__FILE__);
        d.cdUp(); d.cdUp();
...
public:
    ActiveEditorSafetyTest() : QObject() {}
    void run() {
        QDir d(__FILE__);
        d.cdUp(); d.cdUp();
