...
void AutoCompletion::showPathCompletion()
{
	...
	std::wstring autoCompleteEntries;
	{
		...
		for (const QFileInfo& fi : entries)
		{
			...
			autoCompleteEntries += addTrailingSlash(pathToMatch);
			autoCompleteEntries += fi.fileName().toStdWString();
			if (fi.isDir())
				autoCompleteEntries += L"\\";
		}
	}
...
void AutoCompletion::update(int character)
{
	...
	{
		if (_funcCalltip.updateCalltip(character))
			return;
	}
...
const wchar_t* AutoCompletion::getApiFileName()
{
	...
	if (_curLang >= LangType::L_EXTERNAL && _curLang < NppParameters::getInstance().L_END)
	{
		...
		return wmc.char2wchar(NppParameters::getInstance().getELCFromIndex(_curLang - LangType::L_EXTERNAL)->_name.c_str(), CP_UTF8);
	}
	...
	return ScintillaComponent::_langNameInfoArray[static_cast<int>(_curLang)]._langName;
}
