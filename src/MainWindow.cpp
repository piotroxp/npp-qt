...
MainWindow::MainWindow(Application* app) : QMainWindow(nullptr), _app(app)
{
    ...
    // ── ShortcutAdapter (Win32 RegisterHotKey → Qt6 QShortcut) ──────────────
    // The adapter is parented to 'this' so its QShortcut children are destroyed
    // when the main window is destroyed, matching the Win32 semantics where
    // UnregisterHotKey is implicit on WM_DESTROY.
    _shortcutAdapter = new ShortcutAdapter(this);

    // Wire shortcut dispatch → MainWindow command dispatch.
    // When a QShortcut fires, the adapter emits shortcutDispatchRequested;
    // this lambda re-emits it as a bound slot call to dispatchCommand.
    connect(_shortcutAdapter, &ShortcutAdapter::shortcutDispatchRequested,
            this, &MainWindow::dispatchCommand);

    // Register standard file/edit/search/view shortcuts.
    _shortcutAdapter->registerStandardShortcuts();
}
