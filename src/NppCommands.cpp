...
void Notepad_plus::command(int id)
{
	switch (id)
	{
		...
		case IDM_EDIT_INSERT_DATETIME_LONG:
		{
			...
			// GetLocalTime -> QDateTime: &currentTime);
		...
		case IDM_FILE_CONTAININGFOLDERASWORKSPACE:
		{
			...
			// SendMessage(NPPM_) -> Qt: GETFULLCURRENTPATH, CURRENTWORD_MAXLENGTH, reinterpret_cast<qintptr>(currentFile.get()));
		...
		case IDM_EDIT_CUT:
		{
			QWidget* focusedHwnd = QApplication::focusWidget();
			if (focusedHwnd == _pEditView->getHSelf())
			{
				if (_pEditView->hasSelection()) // Cut normally
				{
					_pEditView->execute(0 /* WM_CUT -> QClipboard */);
				}
		...
		case IDM_EDIT_COPY:
		{
			...
			if (focusedHwnd == _pEditView->getHSelf())
			{
				...
				{
					_pEditView->execute(0 /* WM_COPY -> QClipboard */);
				}
			...
			else
			{
				...
				if (finder)  // Search result
					finder->scintillaExecute(0 /* WM_COPY -> QClipboard */);
				else
					// SendMessage(WM_) -> Qt: COPY, 0, 0);
			}
			checkClipboard(); // for enabling possible Paste command
			break;
		}
		...
		case IDM_EDIT_CUT_BINARY:
		{
			...
			if (!// OpenClipboard -> QClipboard: NULL))
				return;
			if (!// EmptyClipboard: )
			{
				// CloseClipboard: ;
		...
		case IDM_EDIT_PASTE:
		{
			...
			if (focusedHwnd == _pEditView->getHSelf())
			{
				...
				qintptr selectionMode = _pEditView->execute(SCI_GETSELECTIONMODE);
		...
		case IDM_EDIT_REDACT_SELECTION:
		{
			...
			{
				quintptr start = _pEditView->execute(SCI_GETSELECTIONNSTART, i);
				qintptr end = _pEditView->execute(SCI_GETSELECTIONNEND, i);
				...
				if (charCount > 0)
				{
					...
					_pEditView->execute(SCI_REPLACETARGET, static_cast<quintptr>(-1), reinterpret_cast<qintptr>(maskStr.c_str()));
				}
			}
		...
		case IDM_EDIT_BEGINENDSELECT_COLUMNMODE:
		{
			...
			// CheckMenuItem -> QAction::setChecked: _mainMenuHandle, id, 0 | (isStarted ? 0 : 0));
			int otherId = (id == IDM_EDIT_BEGINENDSELECT) ? IDM_EDIT_BEGINENDSELECT_COLUMNMODE : IDM_EDIT_BEGINENDSELECT;
			// EnableMenuItem -> QAction::setEnabled: _mainMenuHandle, otherId, 0 | (isStarted ? (0 | 0) : 0));
		}
		break;
		...
		case IDM_EDIT_SORTLINES_LOCALE_DESCENDING:
		{
			...
			if (result.status)
				_nativeLangSpeaker.messageBox(result.tagName.c_str(),
					...
					result.status == 0 ? L"Sort Failed" : L"Sort not performed",
					result.status | 0 | 0, 0, result.message.c_str());
		}
		break;
		...
		case IDM_VIEW_PROJECT_PANEL_3:
		{
			...
			else
			{
				if (!(*pp[idx])->isClosed())
				{
					...
					{
						if (::IsChild((*pp[idx])->getHSelf(), QApplication::focusWidget()))
							// SetFocus -> _pEditView->setFocus();
		...
		case IDM_VIEW_TAB_MOVEBACKWARD:
		{
			...
			// SendMessage(TCM_) -> Qt: GETITEM, currentTabIndex, reinterpret_cast<qintptr>(&tciMove));

			// SendMessage(TCM_) -> Qt: GETITEM, newTabIndex, reinterpret_cast<qintptr>(&tciShift));
			// SendMessage(TCM_) -> Qt: SETITEM, currentTabIndex, reinterpret_cast<qintptr>(&tciShift));

			// SendMessage(TCM_) -> Qt: SETITEM, newTabIndex, reinterpret_cast<qintptr>(&tciMove));

			// SendMessage(TCM_) -> Qt: SETCURSEL, newTabIndex, 0);

			// Notify plugins that the document order has changed
			// NPPM_INTERNAL_DOCORDERCHANGED -> Qt signal/doc tab reorder
	Q_UNUSED(newTabIndex);
		}
		break;

		case IDM_EDIT_DELETE:
			_pEditView->execute(0 /* WM_CLEAR -> QClipboard */);
			break;
		...
		case IDC_EDIT_TOGGLEMACRORECORDING:
		{
			if (_recordingMacro)
			{
				...
				_mainEditView.execute(SCI_SETCURSOR, static_cast<quintptr>(SC_CURSORNORMAL));
				_subEditView.execute(SCI_SETCURSOR, static_cast<quintptr>(SC_CURSORNORMAL));
		...
		{
			// SendMessage(NPPM_) -> Qt: LAUNCHFINDINFILESDLG, 0, 0);
			break;
		}
        ...
        case IDM_LANG_USER_DLG :
        {
		    ...
		    if (!udd->isCreated())
		    {
				...
				if (_isUDDocked)
					// WM_COMMAND IDC_DOCK_BUTTON -> Qt signal
	udd->dockButtonClicked();
		...
		case IDM_EDIT_SPLIT_LINES:
		{
			if (_pEditView->execute(SCI_GETSELECTIONS) == 1)
			{
				...
				{
					auto textWidth = _pEditView->execute(SCI_TEXTWIDTH, STYLE_DEFAULT, reinterpret_cast<qintptr>("P"));
		...
		{
			qintptr n = _pEditView->execute(SCI_GETSELECTIONS);
			if (n > 0)
		...
		case IDM_VIEW_NPC_CCUNIEOL:
		{
			...
			for (auto pView : pViews)
			{
				...
				{
					pView->execute(SCI_STYLESETVISIBLE, static_cast<quintptr>(SCE_ERR_ESCSEQ), static_cast<qintptr>(isChecked));
					pView->execute(SCI_STYLESETVISIBLE, static_cast<quintptr>(SCE_ERR_ESCSEQ_UNKNOWN), static_cast<qintptr>(isChecked));
				}
			}
		...
		case IDM_VIEW_SYNSCROLLH:
		{
			...
			if (_syncInfo._isSynScrollH)
			{
				...
				intptr_t pixel = _mainEditView.execute(SCI_TEXTWIDTH, STYLE_DEFAULT, reinterpret_cast<qintptr>("P"));
		...
		case IDM_VIEW_SUMMARY:
		{
			...
			if (pNativeSpeaker)
			{
				...
				QMessageBox::warning(qobject_cast<QWidget*>(_pPublicInterface->getHSelf()),  characterNumber.c_str(), summaryLabel.c_str(), 0|0);
			}
		}
		break;
		...
		case IDM_SETTING_OPENPLUGINSDIR:
		{
			...
			{
				// ShellExecute -> QDesktopServices: NULL, NULL, pluginHomePath, NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			}
			break;
		}
		...
		case IDM_TOOL_MD5_GENERATEINTOCLIPBOARD:
		{
			if (_pEditView->execute(SCI_GETSELECTIONS) == 1)
			{
				...
				if (strLen)
				{
					...
					_pEditView->execute(SCI_GETSELTEXT, 0, reinterpret_cast<qintptr>(selectedStr));
		...
		{
			QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://notepad-plus-plus.org/", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			break;
		}
		case IDM_PROJECTPAGE :
		{
			QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://github.com/notepad-plus-plus/notepad-plus-plus/", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			break;
		}

		case IDM_ONLINEDOCUMENT:
		{
			QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://npp-user-manual.org/", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			break;
		}
		...
		case IDM_FORUM:
		{
			QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://community.notepad-plus-plus.org/", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			break;
		}
		...
		case IDM_CONFUPDATERPROXY :
		{
			...
			if (ver <= WV_XP)
			{
				...
				if (res == IDYES)
				{
					QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://notepad-plus-plus.org/downloads/", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
				}
			}
			else
			{
		...
		case IDM_LANG_OPENUDLDIR:
		{
			...
			::QDesktopServices::openUrl(QUrl::fromLocalFile(, userDefineLangFolderPath.c_str(), NULL, NULL, Qt::WindowState::WindowActive);
			break;
		}

		case IDM_LANG_UDLCOLLECTION_PROJECT_SITE:
		{
			QDesktopServices::openUrl(QUrl::fromLocalFile(, L"https://github.com/notepad-plus-plus/userDefinedLanguages", NULL, NULL, Qt::WindowState::WindowActiveNORMAL);
			break;
		}
		...
		{
			NppGUI & nppGUI = (NppParameters::getInstance()).getNppGUI();
			// ShowWindow -> QWidget: _pPublicInterface->getHSelf(), nppGUI._isMaximized?Qt::WindowState::WindowMaximized:Qt::WindowState::WindowActive);
		...
		case IDM_PINTAB:
		{
			...
			// SendMessage(WM_) -> Qt: NOTIFY, 0, reinterpret_cast<qintptr>(&nmhdr));
			// SendMessage(WM_) -> Qt: SIZE, 0, 0);
		}
		break;
		...
		default :
			...
			{
				_pluginsManager.relayNppMessages(0 /* WM_COMMAND -> QAction triggered */, id, 0);
			}
