...
void NppCommands::searchFindNext()
{
    ...
    int flags = 0;
    if (opts._isMatchCase) flags |= 0x0004;
    if (opts._isWholeWord) flags |= 0x0002;
    view->send(SCI_SETSEARCHFLAGS, flags);
    view->send(2169, 0, reinterpret_cast<sptr_t>(opts._str2Search.c_str()));  // SCI_SEARCHNEXT
}
...
void NppCommands::searchFindPrev()
{
    ...
    if (opts._isWholeWord) flags |= 0x0002;
    view->send(SCI_SETSEARCHFLAGS, flags);
    view->send(2168, 0, reinterpret_cast<sptr_t>(opts._str2Search.c_str()));  // SCI_SEARCHPREV
}
...
void NppCommands::viewWrapSymbol() { auto* view=activeView(); if(view) view->showEOL(!view->isShownEOL()); }
...
void NppCommands::encodingConvertAnsi()
{
    ...
    if (!view)
        return;
    // For in-place encoding conversion: change Scintilla's code page to ANSI
    view->execute(SCI_SETCODEPAGE, 0);
}
...
void NppCommands::encodingConvertUTF16BE()
{
    ...
    if (!view)
        return;
    // UTF-16 BE: set Scintilla code page to UTF-16 (not directly supported, treat as UTF-8)
    // For true BE conversion, would need to manually byte-swap
    view->execute(SCI_SETCODEPAGE, SC_CP_UTF8);
}
...
void NppCommands::languageUserDefine() { QMessageBox::information(nullptr,"User Defined Language","User Defined Language dialog is not yet implemented."); }
...
void NppCommands::settingsImportPlugin() { QMessageBox::information(nullptr,"Import Plugin","Plugin import is not yet implemented."); }

void NppCommands::settingsImportStyleThemes() { QMessageBox::information(nullptr,"Import Style Theme","Style theme import is not yet implemented."); }

void NppCommands::settingsEditContextMenu() { QMessageBox::information(nullptr,"Edit Context Menu","Context menu editor is not yet implemented."); }
...
void NppCommands::playMacro(const Macro& steps)
{
    ...
    for (const RecordedMacroStep& step : steps) {
        ...
        if (ncStep.isScintillaMacro()) {
            ncStep.playBack();
        } else {
            // Menu command — dispatch through NppCommands
            emit commandExecuted(ncStep._message);
        }
    }
    view->execute(SCI_ENDUNDOACTION);
}
...
void NppCommands::runCmdLineArgs() { QMessageBox::information(nullptr,"Command Line","Command line arguments are set via program launch."); }
...
void NppCommands::windowSortAscending() { windowSortAscending(); }

void NppCommands::windowSortDescending() { windowSortDescending(); }
...
void NppCommands::helpAbout() { helpAbout(); }

void NppCommands::helpDocumentation() { QDesktopServices::openUrl(QUrl("https://notepad-plus-plus.org")); }

void NppCommands::helpHomeSweetHome() { QDesktopServices::openUrl(QUrl("https://notepad-plus-plus.org")); }

void NppCommands::helpOnlineDoc() { QDesktopServices::openUrl(QUrl("https://notepad-plus-plus.org")); }
...
void NppCommands::columnEditor() { auto* view = activeView(); if(view) view->send(2599,1); }
...
void NppCommands::clipboardHistoryPanel() { clipboardHistoryPanel(); }

void NppCommands::charPanel() { charPanel(); }
...
void NppCommands::toggleProjectPanel1() { toggleFileBrowser(); }
void NppCommands::toggleProjectPanel2() { toggleFileBrowser(); }
void NppCommands::toggleProjectPanel3() { toggleFileBrowser(); }
...
{
    QMessageBox::information(nullptr,"Find in Projects","Find in Projects is not yet implemented.");
}
