...
// Non-Windows stub: Windows SEH exception handling is not available
// These stubs allow compilation on Linux; functionality is a no-op

typedef void* ExceptionAddress; // void* on non-Windows

class Win32Exception : public std::exception
{
public:
    static void			installHandler() { /* no-op */ }
    static void			removeHandler()  { /* no-op */ }
    virtual const char* what()  const throw() { return "Win32Exception stub"; }
    ExceptionAddress	where() const         { return nullptr; }
    unsigned int		code()  const         { return 0; }
    void*				info()  const         { return nullptr; }

protected:
    explicit Win32Exception(void* /*info*/) {}
    static void translate(unsigned /*code*/, void* /*info*/) {}

private:
    const char * _event = nullptr;
    ExceptionAddress _location = nullptr;
    unsigned int _code = 0;
    void* _info = nullptr;
};
...
public:
    ...
    ExceptionAddress	badAddress() const { return nullptr; }
private:
    explicit Win32AccessViolation(void* /*info*/) : Win32Exception(nullptr) {}
