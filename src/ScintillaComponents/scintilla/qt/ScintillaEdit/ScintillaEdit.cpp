...
QPair<int, int>ScintillaEdit::find_text(int flags, const char *text, int cpMin, int cpMax) const {
    struct Sci_TextToFindFull ft = {{0, 0}, 0, {0, 0}};
    ft.chrg.cpMin = cpMin;
    ft.chrg.cpMax = cpMax;
    ...
    int start = send(SCI_FINDTEXTFULL, flags, (uptr_t) (&ft));
...
QByteArray ScintillaEdit::get_text_range(int start, int end) const {
    ...
    QByteArray ba(length+1, '\0');
    struct Sci_TextRangeFull tr = {{start, end}, ba.data()};
...
{
    Sci_RangeToFormatFull to_format;

    to_format.hdc = static_cast<Sci_SurfaceID>(static_cast<void*>(target));
    to_format.hdcTarget = static_cast<Sci_SurfaceID>(static_cast<void*>(measure));
    ...
    return send(SCI_FORMATRANGEFULL, draw, reinterpret_cast<sptr_t>(&to_format));
}
