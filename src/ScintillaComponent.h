...
//   SendMessage(hwnd, SCI_*, ...) → send(static_cast<unsigned int>(SCI_)*, wparam, lparam)
...
//   SendMessage(_hSelf, SCI_GETDIRECTFUNCTION, ...) → send(static_cast<unsigned int>(SCI_GETDIRECTFUNCTION), ...)
...
public:
    ...
    // Qt6:   same, via send(static_cast<unsigned int>(SCI_SETDOCPOINTER), ...).
    ...
    void setLineIndent(int line, int indent) { send(static_cast<unsigned int>(SCI_SETLINEINDENTATION), line, indent); }
    ...
    intptr_t currentXOffset() const { return send(static_cast<unsigned int>(SCI_GETXOFFSET)); }
    void setXOffset(long offset) { send(static_cast<unsigned int>(SCI_SETXOFFSET), offset); }
    intptr_t pointXFromPosition(size_t pos) const { return send(static_cast<unsigned int>(SCI_POINTXFROMPOSITION), 0, pos); }
    intptr_t pointYFromPosition(size_t pos) const { return send(static_cast<unsigned int>(SCI_POINTYFROMPOSITION), 0, pos); }
    intptr_t textHeight() const { return send(static_cast<unsigned int>(SCI_TEXTHEIGHT)); }
    ...
    size_t selectionStart() const { return send(static_cast<unsigned int>(SCI_GETSELECTIONSTART)); }
    size_t selectionEnd() const { return send(static_cast<unsigned int>(SCI_GETSELECTIONEND)); }
    Sci_CharacterRangeFull getSelection() const {
        Sci_CharacterRangeFull cr;
        cr.cpMin = static_cast<Sci_Position>(send(static_cast<unsigned int>(SCI_GETSELECTIONSTART)));
        cr.cpMax = static_cast<Sci_Position>(send(static_cast<unsigned int>(SCI_GETSELECTIONEND)));
        return cr;
    }
    ...
    void setBraceMatchIndicator(bool enable);
    void setAutoIndentMode(int mode);  // 0=none, 1=simple, 2=smart
    void setTabIndents(bool tabIndents);
    void setIndentWidth(int width);
    void setFoldMargin(bool show);
    ...
    void convertIndentationToSpacesLine(int line);
    ...
    void zoomIn()  { send(static_cast<unsigned int>(SCI_ZOOMIN)); }
    void zoomOut() { send(static_cast<unsigned int>(SCI_ZOOMOUT)); }
    int zoom() const { return send(static_cast<unsigned int>(SCI_GETZOOM)); }
    void setZoom(int zoomLevel) { send(static_cast<unsigned int>(SCI_SETZOOM), zoomLevel); }

    // --- Read-only state ---

    void setReadOnly(bool ro) { send(static_cast<unsigned int>(SCI_SETREADONLY), ro); }
    bool isReadOnly() const { return send(static_cast<unsigned int>(SCI_GETREADONLY)) != 0; }

    // --- Encoding / EOL ---

    int codepage() const { return _codepage; }
    void setCodepage(int cp) { _codepage = cp; send(static_cast<unsigned int>(SCI_SETCODEPAGE), cp); }
    void setEolMode(int mode) { send(static_cast<unsigned int>(SCI_SETEOLMODE), mode); }
    int eolMode() const { return send(static_cast<unsigned int>(SCI_GETEOLMODE)); }
    void setCRLF() { send(static_cast<unsigned int>(SCI_SETEOLMODE), SC_EOL_CRLF); send(static_cast<unsigned int>(SCI_CONVERTEOLS), SC_EOL_CRLF); }
    ...
    void undo() { send(static_cast<unsigned int>(SCI_UNDO)); }
    void redo() { send(static_cast<unsigned int>(SCI_REDO)); }
    bool canUndo() const { return send(static_cast<unsigned int>(SCI_CANUNDO)) != 0; }
    bool canRedo() const { return send(static_cast<unsigned int>(SCI_CANREDO)) != 0; }

    // --- Search/Find helpers ---

    // Set search flags (SCFIND_*)
    void setSearchFlags(unsigned int flags) { send(static_cast<unsigned int>(SCI_SETSEARCHFLAGS), flags); }
    unsigned int searchFlags() const { return send(static_cast<unsigned int>(SCI_GETSEARCHFLAGS)); }

    // Target range for search
    void setTargetRange(size_t start, size_t end) {
        send(static_cast<unsigned int>(SCI_SETTARGETSTART), start);
        send(static_cast<unsigned int>(SCI_SETTARGETEND), end);
    }
    ...
    void setDocPointer(intptr_t doc) { send(static_cast<unsigned int>(SCI_SETDOCPOINTER), 0, doc); }
    ...
    intptr_t lastZeroBasedLineNumber() const {
        return static_cast<intptr_t>(send(static_cast<unsigned int>(SCI_GETLINECOUNT))) - 1;
    }
    ...
    size_t getLineLength(size_t line) const {
        return send(static_cast<unsigned int>(SCI_LINELENGTH), line);
    }

    // Returns the indentation of the given line in spaces
    int getLineIndent(size_t line) const {
        return static_cast<int>(send(static_cast<unsigned int>(SCI_GETLINEINDENTATION), line));
    }
    ...
    void insertGenericTextFrom(size_t position, const char* text) {
        if (!text) return;
        send(static_cast<unsigned int>(SCI_INSERTTEXT), static_cast<uptr_t>(position),
             reinterpret_cast<sptr_t>(text));
    }

    // Document map / fold helpers
    QRect getClientRect() const { return rect(); }
    size_t getCurrentDocLen() const { return static_cast<size_t>(send(static_cast<unsigned int>(SCI_GETLENGTH))); }
    ...
    // --- Comment prefix utility ---
    // Returns the line-comment prefix string for a given language type.
    // Returns an empty QString if the language has no line-comment style.
    static QString getCommentPrefix(LangType lang);
...
public:
    ...
    void setPositionReached(intptr_t pos) { send(static_cast<unsigned int>(SCI_SETCURRENTPOS), pos); }
