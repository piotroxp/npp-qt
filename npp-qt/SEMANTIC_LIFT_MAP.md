...
> Last updated: 2026-07-02 19:50 UTC
...
| FindReplaceDlg | ScintillaComponent/FindReplaceDlg.h/cpp | src/FindReplaceDlg.h/cpp | 140+282 | ✅ DONE | Full QWidget; findNext/replace/mark/filter; QDirIterator for FIF (1083 lines) |
...
| UserDefineDialog | ScintillaComponent/UserDefineDialog.h/cpp | src/UserDefineDialog.h/cpp | 61+182 | ⚠️ STUB | Keywords/stylers/folders/comments; real-time preview pending (81 lines — incomplete) |
...
└── src/
    ...
    └── WinControls/
        ...
        ├── AnsiCharPanel.h/cpp ✅
        ├── ColourPicker.h/cpp  ✅
        ├── WordStyleDlg.h/cpp   ✅ (LIFTED — 985 lines, full UDL styler dialog)
        ├── RunMacroDlg.h/cpp   ✅ (LIFTED — 190 lines, macro runner dialog)
...
- **Build**: CMakeLists.txt has 104 entries (~82 unique), all confirmed on disk. All 6 core files ported to Qt6: Parameters.cpp (8929L), NppIO.cpp (2961L), NppCommands.cpp (4513L), NppBigSwitch.cpp (4305L), Notepad_plus.cpp (9331L), main.cpp (875L). Win32 types: 0 HWND/0 HDC/0 HINSTANCE in portable code. Binary: `build/npp-qt` (3.9MB), MD5 1b15f29c.
- **Tests**: 109 subtests pass via Qt Test framework (`tests/` directory). Targets: Utf8_16, NppDarkMode, ColourPicker, LesDlgs, CustomFileDialog, DirectoryWatcher, ImageListSet. Run: `cd build && ctest --output-on-failure`.
...
- **Remaining stubs**: `NppCommandsSlots_stub.cpp` (~15 no-op stubs for linker safety), `StubDialogs.h`. Real implementations pending: UserDefineDialog (81 lines, incomplete), ScintillaEditBase (abstract wrapper pending Scintilla layer). All stubs are linker-safe no-ops, not blocking the build.

### P2 Items (Non-Blocking)

| Item | Status | Notes |
|------|--------|-------|
| Plugin NPP API | ⚠️ Partial | PluginInterface.h complete (167L); PluginsManager.cpp substantial (492L); NPPM_* messages wired in NppBigSwitch; full plugin load/unload not end-to-end tested |
| i18n .qm files | ⚠️ Missing | `localization.cpp` uses Qt Linguist; NppXml Qt DOM wrapper exists; need `lrelease` to generate .qm from .ts source files |
| Linux dark mode specifics | ⚠️ Partial | NppDarkMode.h/cpp exists (4542L); `d0590e8 fix: NppDarkMode Linux palette propagation + tone colours` applied; Linux-specific palette tweaks may be needed |
