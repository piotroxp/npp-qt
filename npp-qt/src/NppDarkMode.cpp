...
#ifdef Q_OS_LINUX
    // Check GTK dark mode preference first (GNOME, XFCE, etc.)
    const char* gtkTheme = qgetenv("GTK_THEME");
    if (gtkTheme) {
        QString theme = QString::fromLatin1(gtkTheme).toLower();
        if (theme.contains("dark"))
            return QStringLiteral("dark");
        return QString::fromLatin1(gtkTheme);
    }
    // Check GNOME color scheme preference
    const char* gnomeScheme = qgetenv("GNOME_COLOR_SCHEME");
    if (gnomeScheme) {
        QString scheme = QString::fromLatin1(gnomeScheme).toLower();
        if (scheme.contains("dark"))
            return QStringLiteral("dark");
        if (scheme.contains("light"))
            return QStringLiteral("light");
    }
    // Fallback: check Qt platform theme name
    QByteArray qtTheme = qgetenv("QT_QPA_PLATFORMTHEME");
    if (!qtTheme.isEmpty()) {
        return QString::fromLatin1(qtTheme.constData());
    }
    // Fallback: check XDG current desktop
    QByteArray xdgTheme = qgetenv("XDG_CURRENT_DESKTOP");
    if (!xdgTheme.isEmpty()) {
        return QString::fromLatin1(xdgTheme.constData());
    }
    // Final fallback: palette luminance check
...
// setDarkMode(bool) — primary dark-mode switcher.
// Mirrors the Win32 NppDarkMode::setDarkMode(bool).
// Applies a dark Fusion palette + dark scrollbar styles to the entire Qt widget
// tree.  Called by NppParameters::setDarkMode() and Notepad_plus::doDarkMode().
void NppDarkMode::setDarkMode(bool enable)
{
    ...
    if (enable) {
        ...
        // Dark scrollbar CSS is applied in setDarkScrollBar() called on-demand.
        // We apply it globally to the qApp stylesheet so all widgets inherit it.
        qApp->setStyleSheet(
            "QScrollBar:vertical { background: #2a2a2a; width: 14px; }"
            "QScrollBar::handle:vertical { background: #4a4a4a; min-height: 20px; border-radius: 6px; }"
            "QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }"
...
// ── Scintilla dark-mode integration ──────────────────────────────────────────
// setScintillaFontColor / setScintillaBgColor — apply dark foreground /
// background colours to a Scintilla editor widget via SCI_STYLESETFORE /
// SCI_STYLESETBACK for the default style (STYLE_DEFAULT = 0).
// Called from NppParameters when dark mode is toggled so all open editors
// update their default text colour to match the dark palette.
void NppDarkMode::setScintillaFontColor(QsciScintilla* sci, QRgb fgColor)
{
    if (!sci)
        return;
    sci->SendScintilla(QsciScintillaBase::SCI_STYLESETFORE,
                       0 /* STYLE_DEFAULT */,
                       fgColor);
}

void NppDarkMode::setScintillaBgColor(QsciScintilla* sci, QRgb bgColor)
{
    if (!sci)
        return;
    sci->SendScintilla(QsciScintillaBase::SCI_STYLESETBACK,
                       0 /* STYLE_DEFAULT */,
                       bgColor);
}
