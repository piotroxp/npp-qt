...
{
    auto originalStartPos = sci->send(npp_sci::SCI_GETTARGETSTART);
    auto originalEndPos = sci->send(npp_sci::SCI_GETTARGETEND);

    intptr_t firstLine = sci->send(npp_sci::SCI_GETFIRSTVISIBLELINE);
    intptr_t nbLineOnScreen = sci->send(npp_sci::SCI_LINESONSCREEN);
    ...
    for (intptr_t currentLine = firstLine; currentLine < lastLine; ++currentLine) {
        intptr_t docLine = sci->send(npp_sci::SCI_DOCLINEFROMVISIBLE, currentLine);
        if (docLine == prevDocLineChecked) continue;
        prevDocLineChecked = docLine;

        size_t startPos = sci->send(npp_sci::SCI_POSITIONFROMLINE, docLine);
        size_t endPos = sci->send(npp_sci::SCI_POSITIONFROMLINE, docLine + 1);
        if (endPos == size_t(-1)) {
            endPos = sci->send(npp_sci::SCI_GETLENGTH);
    ...
    sci->send(npp_sci::SCI_SETTARGETSTART, originalStartPos);
    sci->send(npp_sci::SCI_SETTARGETEND, originalEndPos);
}
...
void SmartHighlighter::highlightView(ScintillaComponent* sci, ScintillaComponent* unfocusView)
{
    ...
    const NppGUI& nppGUI = NppParameters::getInstance().getNppGUI();
    if (!nppGUI._enableSmartHilite || sci->send(npp_sci::SCI_GETSELECTIONEMPTY)) {
    ...
    auto curPos = sci->send(npp_sci::SCI_GETCURRENTPOS);
    ...
    if (isWordOnly) {
        auto wordStart = sci->send(npp_sci::SCI_WORDSTARTPOSITION, curPos, true);
        auto wordEnd = sci->send(npp_sci::SCI_WORDENDPOSITION, wordStart, true);
