...
static const QString defExtArray[nbSupportedLang][nbExtMax] = {
    ...
    {QStringLiteral("c, c++, objc"),
     ...
     QStringLiteral(".vcproj"),
     QStringLiteral(".props"),
     QStringLiteral(".vsprops"),
...
bool RegExtDlg::deleteExts(const QString &ext2Delete)
{
    ...
    // Qt6/Linux: Remove from ~/.config/mimeapps.list if present
    QString mimeListPath = QDir::homePath() + QStringLiteral("/.config/mimeapps.list");
    Q_EMIT extensionRemoved(ext2Delete);
    Q_UNUSED(ext2Delete);  // MIME type removal from desktop file is complex — handled via desktop env tools
    return true;
}
...
void RegExtDlg::writeNppPath()
{
    ...
    //        + HKEY_CLASSES_ROOT\nppName\DefaultIcon
    // Qt6/Linux: Write ~/.local/share/applications/npp.desktop
    ...
    QString desktopContent = QStringLiteral(
        ...
        "Type=Application\n"
        "Name=Notepad++ (npp-qt)\n"
        "Comment=A Qt6 port of Notepad++ text editor\n"
        "Exec=%1\n"
        "Icon=text-editor\n"
        "Terminal=false\n"
        "Categories=TextEditor;Development;\n"
        "MimeType=text/plain;text/css;text/html;text/x-csrc;text/x-c++src;text/x-java;text/x-python;text/x-script;text/xml;\n"
    ).arg(appPath);

    // Write to ~/.local/share/applications/
    QString desktopDir = QDir::homePath() + QStringLiteral("/.local/share/applications");
    QDir().mkpath(desktopDir);
    QString desktopPath = desktopDir + QStringLiteral("/npp-qt.desktop");
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        out << desktopContent;
        file.close();
        qDebug() << "RegExtDlg: wrote desktop file to" << desktopPath;
    } else {
        qDebug() << "RegExtDlg: failed to write desktop file to" << desktopPath;
    }
}
