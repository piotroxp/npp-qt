...
// ScintillaEditView is used by playBack() — include for the type definition
#include "../ScintillaComponent.h"
#include <QString>
#include <QVector>
// NppSciCompat.h is included via ScintillaComponent.h above (pragma-once allows
// re-inclusion from shortcut.h). Do NOT include it again directly — that would
// hit pragma once and skip its SC_UPDATE_* / SCK_* undefs, leaving those macros
// still defined when npp_sci:: constexpr values are declared (causing
// "expected ';' before numeric constant" errors). NppSciCompat.h's #undef
// directives are inside its include guard and execute on every inclusion.

// menuCmdID.h defines #define IDM_* macros. keys.h declares const int IDM_*.
// menuCmdID.h MUST come first so its #define values are active when keys.h
// tries to declare const int IDM_* (which would otherwise be macro-expanded).
#include "menuCmdID.h"

#undef IDM

// shortcut.cpp (and other files) include menuCmdID.h directly AFTER including
// shortcut.h. After #undef IDM, the direct include hits the include guard and
// skips menuCmdID.h's content — leaving IDM_* undefined for shortcut.cpp.
// Re-include menuCmdID.h HERE (after #undef IDM) so shortcut.cpp's direct
// #include sees the content and IDM_* macros are available for C++ use.
// IDM itself stays undefined (so C++ code can declare variables named IDM_*),
// but all IDM_* sub-category macros are still defined for switch/case use.
#undef MENU_CMD_ID_H
#include "menuCmdID.h"
...
public:
    ...
    void setCategoryFromMenu(unsigned long cmdID);
...
struct RecordedMacroStep {
    ...
    void playBack(ScintillaEditView* view);
};
