	...
	: QTabWidget(parent), _pView(nullptr), _iconListIndexChoice(-1)
{
	...
	// tabBarMoved is emitted when tabs are drag-reordered
	connect(tabBar(), &QTabBar::tabBarMoved, this, &DocTabView::tabMoved);
}

void DocTabView::tabMoved(int /*from*/, int /*to*/)
{
	// Emit signal so Notepad_plus can notify plugins via NPPN_DOCORDERCHANGED
