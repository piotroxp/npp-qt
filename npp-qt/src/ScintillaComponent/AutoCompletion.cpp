...
void AutoCompletion::getWordArray(std::vector<std::string>& wordArray, const char* beginChars, const char* excludeChars) const
{
	...
	intptr_t _ac_fpos = 0;
	intptr_t posFind = _pEditView->searchInTarget(expr, _ac_fpos, static_cast<intptr_t>(docLength));
	...
	while (posFind >= 0)
	{
		...
		posFind = _pEditView->searchInTarget(expr, _ac_tpos, static_cast<intptr_t>(docLength));
	}
}
...
void AutoCompletion::getCloseTag(char* closeTag, size_t closeTagLen, size_t caretPos, bool isHTML) const
{
	...
	const intptr_t targetStart = _pEditView->searchInTarget(_ac_tag, _ac_cpos, static_cast<intptr_t>(0));
	if (targetStart < 0)
		return;
...
void AutoCompletion::insertMatchedChars(int character, const MatchedPairConf& matchedPairConf)
{
	...
	for (const auto& matchedPair : matchedPairConf._matchedPairs)
	{
		...
		{
			const char userMatchedChar[2]{ static_cast<char>(matchedPair.second), '\0' };
...
bool AutoCompletion::setLanguage(LangType language)
{
	...
	QString apiFileName = QString::fromWCharArray(getApiFileName());
	...
	if (_funcCompletionActive)
	{
		...
		if (!elem.isNull())
		{
			...
			if (val.compare(u"no"_s, Qt::CaseInsensitive) == 0)
			{
				_ignoreCase = false;
...
QString AutoCompletion::getApiFileName() const
{
	...
	if (_curLang >= LangType::L_EXTERNAL && _curLang < NppParameters::getInstance().L_END)
	{
		return QString::fromStdString(
			NppParameters::getInstance().getELCFromIndex(_curLang - LangType::L_EXTERNAL)->_name);
	}

	if (_curLang > LangType::L_EXTERNAL)
		_curLang = LangType::L_TEXT;

	if (_curLang == LangType::L_JAVASCRIPT)
		_curLang = LangType::L_JS_EMBEDDED;

	return QString::fromWCharArray(ScintillaComponent::_langNameInfoArray[static_cast<int>(_curLang)]._langName);
}
