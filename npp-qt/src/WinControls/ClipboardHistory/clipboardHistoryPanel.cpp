...
//   LBN_DBLCLK → paste            → QListWidget::itemDoubleClicked signal
...
#include <windows.h>
#else
// Only WM_INITDIALOG is still referenced (for dialog init); defined in NppConstants.h
constexpr int IDC_LIST_CLIPBOARD = 0;  // used for setObjectName() in init()
#endif
...
QString ClipboardHistoryPanel::buildDisplayString(const ClipboardDataInfo& cbd)
{
    ...
    if (cbd._isBinaryContained) {
        // Win32: treated as UTF-8 via WcharMbcsConvertor::char2wchar(cp=CP_UTF8)
...
void ClipboardHistoryPanel::pasteItemAt(int row)
{
    ...
    if (cbd._isBinaryContained) {
        ...
        //       ? (SCI_GETCODEPAGE == CP_UTF8 ? CP_UTF8 : 0)
        //       : CP_UTF8;
        ...
        QByteArray encoded = (codepage == CP_UTF8)
            ? text.toUtf8()
...
// =============================================================================
// run_dlgProc — Dialog message dispatcher
// Win32 messages are handled by Qt slots/overrides (onClipboardChanged,
// onItemDoubleClicked, resizeEvent, etc.).  Only the custom NPP message
// NPPM_INTERNAL_REFRESHDARKMODE requires explicit handling here.
// =============================================================================
                                             ...
                                             intptr_t wParam, intptr_t lParam)
{
    ...
    case NPPM_INTERNAL_REFRESHDARKMODE: {
        // Custom NPP message: refresh dark scrollbar on theme change
        NppDarkMode::instance().setDarkScrollBar(_listWidget);
        return 1;
    }
