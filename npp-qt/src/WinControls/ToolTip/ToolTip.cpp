...
void ToolTip::showTooltip(const QString& text, int x, int y)
{
    ...
    // Keep on screen (Qt6: use primaryScreen instead of QDesktopWidget)
    QScreen* screen = QApplication::primaryScreen();
    if (x + tw > screen->geometry().width())
        x = screen->geometry().width() - tw - 4;
    if (y + th > screen->geometry().height())
        y = screen->geometry().height() - th - 4;
