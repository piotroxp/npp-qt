...
void ToolBarIcons::create(const QSize& iconSize)
{
    ...
    for (const auto& unit : _buttonUnits) {
        ...
        // Dark mode variants — create a proper dark-disabled icon
        _iconListVector[HLIST_DEFAULT_DM].append(darkIcon);

        // Dark-disabled variant: start from dark icon, apply grayscale + reduced brightness
        QPixmap darkPm = darkIcon.pixmap(iconSize);
        QPixmap darkGrayPm(iconSize);
        darkGrayPm.fill(Qt::transparent);
        QPainter dgp(&darkGrayPm);
        dgp.drawPixmap(0, 0, darkPm);
        dgp.setCompositionMode(QPainter::CompositionMode_SourceOver);
        // Apply grayscale by averaging R, G, B channels
        dgp.setCompositionMode(QPainter::CompositionMode_Clear);
        dgp.fillRect(darkGrayPm.rect(), Qt::transparent);
        dgp.setCompositionMode(QPainter::CompositionMode_SourceOver);
        // Draw grayscale version of dark icon
        QImage grayImg = darkPm.toImage().convertToFormat(QImage::Format_ARGB32);
        for (int y = 0; y < grayImg.height(); ++y) {
            for (int x = 0; x < grayImg.width(); ++x) {
                QRgb px = grayImg.pixel(x, y);
                int gray = qGray(px);
                // Reduce brightness to ~50% for disabled look, keep alpha
                gray = qBound(0, gray * 2 / 3, 255);
                grayImg.setPixel(x, y, qRgba(gray, gray, gray, qAlpha(px)));
            }
        }
        dgp.drawImage(0, 0, grayImg);
        dgp.end();
        QIcon darkDisabledIcon;
        darkDisabledIcon.addPixmap(darkGrayPm);
        _iconListVector[HLIST_DISABLE_DM].append(darkDisabledIcon);

        _iconListVector[HLIST_DEFAULT_DM2].append(darkIcon);
        _iconListVector[HLIST_DISABLE_DM2].append(darkDisabledIcon);
    }
}
