...
void ProjectPanel::enumWorkSpaceFiles(std::vector<QString>& paths, int type) const
{
    ...
    // Walk the workspace tree and collect all file paths from ProjectTreeItems
    QTreeWidgetItem* root = _treeWidget->invisibleRootItem();
    for (int i = 0; i < root->childCount(); ++i) {
        collectFilesRecursive(root->child(i), paths);
    }
}

void ProjectPanel::collectFilesRecursive(QTreeWidgetItem* item, std::vector<QString>& paths) const
{
    auto* projItem = dynamic_cast<ProjectTreeItem*>(item);
    if (projItem) {
        QString fp = projItem->fullPath();
        if (!fp.isEmpty()) {
            // It's a file item — check if it's visible/active
    ...
    for (int i = 0; i < item->childCount(); ++i) {
        collectFilesRecursive(item->child(i), paths);
    }
}
