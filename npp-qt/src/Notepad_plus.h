...
// NppSciCompat.h is included via ScintillaEditView.h (below) → ScintillaComponent.h.
// Do NOT include it directly here — that would be blocked by pragma once and skip
// its SC_UPDATE_* undefs, leaving SC_UPDATE_NONE still macro-defined.
// Sci_CharacterRangeFull and npp_sci::SCI_* constants come from NppSciCompat.h.
...
#include "WinControls/ShortcutMapper.h"
#include "MISC/Common/lastRecentFileList.h"
...
public:
    ...
    // Handle NPPM_* message from plugin (replaces Win32 SendMessage-based routing)
    // Returns message-specific result (usually intptr_t; 0 for void commands)
    intptr_t handleNppMessage(unsigned int msg, uintptr_t wParam, intptr_t lParam);
...
private:
    ...
    CmdLineArgsDlg& getCmdLineArgsDlg() { return _cmdLineArgsDlg; }
	RunDlg _runDlg;
public:
    ...
    ColumnEditorDlg& getColumnEditorDlg() { return _colEditorDlg; }
