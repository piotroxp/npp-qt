...
void GoToLineDlg::updateLinesNumbers()
{
    ...
    if (_mode == go2line) {
        ...
        long long limit = sci->send(npp_sci::SCI_GETLINECOUNT);
        _currentLabel->setText(QString::number(current));
        _lastLabel->setText(QString::number(limit));
    } else {
        long long current = sci->send(npp_sci::SCI_GETCURRENTPOS);
        long long limit = sci->send(npp_sci::SCI_GETLENGTH);
        if (limit > 0) limit--;
        _currentLabel->setText(QString::number(current));
        _lastLabel->setText(QString::number(limit));
    }
...
void GoToLineDlg::accept()
{
    ...
    ScintillaComponent* sci = *_ppEditView;
    sci->send(npp_sci::SCI_SETMODEVENTMASK, 0);
    long long line = getLine();
    if (line != -1) {
        if (_mode == go2line) {
            sci->send(npp_sci::SCI_ENSUREVISIBLE, line - 1);
            sci->send(npp_sci::SCI_GOTOLINE, line - 1);
        } else {
            size_t posToGoto = 0;
            if (line > 0) {
                auto before = sci->send(npp_sci::SCI_POSITIONBEFORE, line);
                posToGoto = sci->send(npp_sci::SCI_POSITIONAFTER, before);
            }
            auto sci_line = sci->send(npp_sci::SCI_LINEFROMPOSITION, posToGoto);
            sci->send(npp_sci::SCI_ENSUREVISIBLE, sci_line);
            sci->send(npp_sci::SCI_GOTOPOS, posToGoto);
        }
    }
    sci->send(npp_sci::SCI_SETMODEVENTMASK, 0x1FF);
    QDialog::accept();
}
