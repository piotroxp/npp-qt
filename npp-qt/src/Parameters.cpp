...
// NppSciCompat.h is included via Parameters.h → shortcut.h → ScintillaComponent.h.
// Do NOT include it directly here — that would be blocked by pragma once and
// skip its SC_UPDATE_* undefs, leaving SC_UPDATE_NONE etc. still macro-defined.
...
// ToolBar: depends on Notepad_plus (circular dependency — ToolBar created in MainWindow post-Notepad_plus construction)

#include "keys.h"
#include "localization.h"
// localizationString: maps language IDs to localized language names (handled by NppParameters::getLangIDFromStr / getLangDescription)
#include "menuCmdID.h"
...
void ThemeSwitcher::addDefaultThemeFromXml(const std::wstring& xmlFullPath)
{
	_themeList.push_back(std::make_pair(_defaultThemeLabel, xmlFullPath));
}

std::pair<std::wstring, std::wstring>& ThemeSwitcher::getElementFromIndex(size_t index)
{
...
{
	// SetWindowLongPtr with GWL_EXSTYLE WS_EX_LAYERED removed → restore opaque window
	if (hwnd != nullptr)
		hwnd->setWindowOpacity(1.0);
}

void NppParameters::setTransparent(QWidget* hwnd, int percent)
{
	// SetLayeredWindowAttributes: mirror via QWidget::setWindowOpacity()
	// percent is 0-255 (Win32 LWA_ALPHA), map to 0.0-1.0 for Qt
	if (percent > 255)
		percent = 255;
	else if (percent < 0)
		percent = 0;
	if (hwnd)
		hwnd->setWindowOpacity(percent / 255.0);
}
...
void NppParameters::feedMacros(const NppXml::Element& element)
{
    ...
    if (macrosRoot.isNull())
        return;

    for (NppXml::Element macroNode = NppXml::firstChildElement(macrosRoot, "Macro");
         !macroNode.isNull();
         macroNode = NppXml::nextSiblingElement(macroNode, "Macro")) {

        QString name = NppXml::attribute(macroNode, "name");
        if (name.isEmpty())
            continue;
        ...
        sc._name = name;
        sc._menuName = name;
        sc._keyCombo._isCtrl = NppXml::attribute(macroNode, "Ctrl") == "yes";
        sc._keyCombo._isAlt = NppXml::attribute(macroNode, "Alt") == "yes";
        sc._keyCombo._isShift = NppXml::attribute(macroNode, "Shift") == "yes";
        bool ok = false;
        sc._keyCombo._key = static_cast<quint32>(NppXml::attribute(macroNode, "Key").toInt(&ok));
        if (!ok) sc._keyCombo._key = 0;

        Macro macroSteps;
        getActions(macroNode, macroSteps);

        if (macroSteps.empty())
            continue;  // skip empty macros

        const int cmdID = ID_MACRO + static_cast<int>(_macros.size());
        QPointer<MacroShortcut> macroSc = new MacroShortcut(sc, macroSteps, cmdID, nullptr);
        _macros.push_back(macroSc);
    }
}
...
void NppParameters::getActions(const NppXml::Element& element, Macro& macro)
{
    ...
    for (NppXml::Element actionNode = NppXml::firstChildElement(element, "Action");
        ...
        RecordedMacroStep step;
        bool ok = false;

        int typeVal = NppXml::intAttribute(actionNode, "type", ok);
        if (ok && typeVal >= 0 && typeVal < 4)
            step._macroType = static_cast<RecordedMacroStep::MacroTypeIndex>(typeVal);

        step._message = static_cast<uintptr_t>(NppXml::intAttribute(actionNode, "message", ok));
        step._wParameter = static_cast<uintptr_t>(NppXml::attribute(actionNode, "wParam").toULongLong(&ok));
        step._lParameter = static_cast<uintptr_t>(NppXml::attribute(actionNode, "lParam").toULongLong(&ok));
        step._sParameter = NppXml::attribute(actionNode, "sParam");
...
{
    // No-op: GUI parameters are loaded from user preferences via QSettings
    Q_UNUSED(element);
}
...
// ============================================================================
// StyleArray::addStyler — parse <WidgetStyle> XML node into a Style
// ============================================================================
void StyleArray::addStyler(int styleID, const NppXml::Element& styleNode)
{
    ...
    if (isUser) {
        ...
        if (styleID >= 24 || findByID(styleID))  // SCE_USER_STYLE_TOTAL_STYLES = 24
            return;
    }
    ...
    // name attribute
    const QString nameAttr = NppXml::attribute(styleNode, "name");
    if (!nameAttr.isEmpty()) {
        s._styleDesc = nameAttr.toStdWString();
    }

    // fgColor: 6-char hex RGB, e.g. "FF0000"
    const QString fgStr = NppXml::attribute(styleNode, "fgColor");
    if (!fgStr.isEmpty()) {
        bool ok = false;
        unsigned long val = fgStr.toULong(&ok, 16);
        if (ok) {
            s._fgColor = qRgb((val >> 16) & 0xFF, (val >> 8) & 0xFF, val & 0xFF);
        }
    }

    // bgColor
    const QString bgStr = NppXml::attribute(styleNode, "bgColor");
    if (!bgStr.isEmpty()) {
        bool ok = false;
        unsigned long val = bgStr.toULong(&ok, 16);
        if (ok) {
            s._bgColor = qRgb((val >> 16) & 0xFF, (val >> 8) & 0xFF, val & 0xFF);
        }
    }

    // colorStyle
    const QString csStr = NppXml::attribute(styleNode, "colorStyle");
    if (!csStr.isEmpty()) {
        bool ok = false;
        s._colorStyle = csStr.toInt(&ok);
        if (!ok) s._colorStyle = COLORSTYLE_ALL;
    }

    // fontName
    const QString fnStr = NppXml::attribute(styleNode, "fontName");
    if (!fnStr.isEmpty()) {
        s._fontName = fnStr.toStdWString();
        s._isFontEnabled = true;
    }

    // fontStyle (0=none, 1=bold, 2=italic, 4=underline)
    const QString fsStr = NppXml::attribute(styleNode, "fontStyle");
    if (!fsStr.isEmpty()) {
        bool ok = false;
        s._fontStyle = fsStr.toInt(&ok);
        if (!ok) s._fontStyle = STYLE_NOT_USED;
    }

    // fontSize
    const QString fszStr = NppXml::attribute(styleNode, "fontSize");
    if (!fszStr.isEmpty()) {
        bool ok = false;
        s._fontSize = fszStr.toInt(&ok);
        if (!ok) s._fontSize = STYLE_NOT_USED;
    }

    // nesting (indent guides)
    const QString nestStr = NppXml::attribute(styleNode, "nesting");
    if (!nestStr.isEmpty()) {
        bool ok = false;
        s._nesting = nestStr.toInt(&ok);
    }

    // keywordClass (instre1/type1/etc. → LANG_INDEX_*)
    const QString kwStr = NppXml::attribute(styleNode, "keywordClass");
    if (!kwStr.isEmpty()) {
        s._keywordClass = getKwClassFromName(qPrintable(kwStr));
    }
}

// ============================================================================
// LexerStylerArray::addLexerStyler — parse <LexerType> XML node into a LexerStyler
// ============================================================================
void LexerStylerArray::addLexerStyler(const char* lexerName, const char* lexerDesc,
                                       const char* lexerUserExt, const NppXml::Element& lexerNode)
{
    _lexerStylerVect.emplace_back();
    LexerStyler& ls = _lexerStylerVect.back();

    if (lexerName)
        ls.setLexerName(QString::fromUtf8(lexerName).toStdWString().c_str());
    if (lexerDesc)
        ls.setLexerDesc(QString::fromUtf8(lexerDesc).toStdWString().c_str());
    if (lexerUserExt)
        ls.setLexerUserExt(QString::fromUtf8(lexerUserExt).toStdWString().c_str());

    // Parse <WordsStyle> children into styles
    if (!lexerNode.isNull()) {
        NppXml::Element wordsStyle = NppXml::firstChildElement(lexerNode, "WordsStyle");
        while (!wordsStyle.isNull()) {
            bool ok = false;
            int styleID = NppXml::intAttribute(wordsStyle, "styleID", ok);
            if (ok && styleID >= 0) {
                ls.addStyler(styleID, wordsStyle);
            }
            wordsStyle = NppXml::nextSiblingElement(wordsStyle, "WordsStyle");
        }
    }
}

// ============================================================================
// NppParameters::feedStylerArray — load <LexerStyles> from config XML
// Loads lexer definitions (keywords, extensions) and global style defaults
// ============================================================================
bool NppParameters::feedStylerArray(const NppXml::Element& element)
{
    if (element.isNull())
    ...
    NppXml::Element lexerStylesRoot = NppXml::firstChildElement(element, "LexerStyles");
    if (lexerStylesRoot.isNull())
        return false;

    // <LexerType> nodes — one per lexer
    NppXml::Element lexerType = NppXml::firstChildElement(lexerStylesRoot, "LexerType");
    while (!lexerType.isNull()) {
        const QString lexerName = NppXml::attribute(lexerType, "name");
        const QString lexerDesc = NppXml::attribute(lexerType, "desc");
        const QString lexerExt  = NppXml::attribute(lexerType, "ext");

        if (!lexerName.isEmpty()) {
            _lexerStylerVect.addLexerStyler(
                qPrintable(lexerName),
                qPrintable(lexerDesc),
                qPrintable(lexerExt),
                lexerType
            );

            // Excluded languages
            if (NppXml::attribute(lexerType, "excluded") == "yes") {
                int extIdx = getExternalLangIndexFromName(lexerName.toStdWString().c_str());
                if (extIdx >= 0) {
                    _nppGUI._excludedLangList.push_back(static_cast<LangType>(extIdx + L_EXTERNAL));
                }
            }
        }
        lexerType = NppXml::nextSiblingElement(lexerType, "LexerType");
    }

    // Sort lexers alphabetically (Search result last)
    _lexerStylerVect.sort();

    // <GlobalStyles> — default styles for all lexers
    NppXml::Element globalRoot = NppXml::firstChildElement(lexerStylesRoot, "GlobalStyles");
    if (!globalRoot.isNull()) {
        NppXml::Element widgetStyle = NppXml::firstChildElement(globalRoot, "WidgetStyle");
        while (!widgetStyle.isNull()) {
            const QString styleName = NppXml::attribute(widgetStyle, "name");
            // Skip if already in global styles
            if (styleName.isEmpty() || !_widgetStyleArray.findByName(styleName.toStdWString())) {
                bool ok = false;
                int styleID = NppXml::intAttribute(widgetStyle, "styleID", ok);
                if (ok && styleID >= 0) {
                    _widgetStyleArray.addStyler(styleID, widgetStyle);
                }
            }
            widgetStyle = NppXml::nextSiblingElement(widgetStyle, "WidgetStyle");
        }
    }
...
// writeStyle2Element — implemented below writeStyles
...
bool NppParameters::writeColumnEditorSettings() {
    // Column editor settings are persisted via NppSettings (QSettings)
    // No separate XML needed — just trigger a settings sync
    // NppSettings::instance().sync() is called by the settings dialog
    return true;
}
void NppParameters::writeShortcuts()
{
    if (!_isAnyShortcutModified && _macros.empty())
        return;
    ...
    // --- Customized Internal Commands (shortcuts) ---
    if (_isAnyShortcutModified) {
        NppXml::Element oldCmdRoot = NppXml::firstChildElement(root, "InternalCommands");
        if (oldCmdRoot)
            NppXml::deleteChild(root, oldCmdRoot);

        NppXml::Element cmdRoot = NppXml::createChildElement(root, "InternalCommands");
        for (size_t idx : std::as_const(_customizedShortcuts)) {
            if (idx >= _shortcuts.size())
                continue;
            insertCmd(cmdRoot, _shortcuts[idx]);
        }
    }

    // --- Macros ---
    NppXml::Element oldMacros = NppXml::firstChildElement(root, "Macros");
    if (oldMacros)
        NppXml::deleteChild(root, oldMacros);
    if (!_macros.empty()) {
        NppXml::Element macrosRoot = NppXml::createChildElement(root, "Macros");
        for (const QPointer<MacroShortcut>& macroSc : std::as_const(_macros)) {
            if (macroSc.isNull())
                continue;
            NppXml::Element macroRoot = NppXml::createChildElement(macrosRoot, "Macro");
            NppXml::setAttribute(macroRoot, "name", macroSc->getMenuName());
            const KeyCombo& key = macroSc->getKeyCombo();
            ...
            NppXml::setAttribute(macroRoot, "Key", QString::number(key._key));
            for (const RecordedMacroStep& step : macroSc->getMacro()) {
                NppXml::Element actionNode = NppXml::createChildElement(macroRoot, "Action");
                NppXml::setAttribute(actionNode, "type", QString::number(static_cast<int>(step._macroType)));
                NppXml::setAttribute(actionNode, "message", QString::number(step._message));
                NppXml::setAttribute(actionNode, "wParam", QString::number(static_cast<unsigned long long>(step._wParameter)));
                NppXml::setAttribute(actionNode, "lParam", QString::number(static_cast<unsigned long long>(step._lParameter)));
                NppXml::setAttribute(actionNode, "sParam", step._sParameter);
            }
        }
    }

    // --- User Defined Commands ---
    NppXml::Element oldUserCmd = NppXml::firstChildElement(root, "UserDefinedCommands");
    if (oldUserCmd)
        NppXml::deleteChild(root, oldUserCmd);

    // --- Save to file ---
    if (_pXmlShortcutDoc && !_shortcutsPath.empty())
        static_cast<void>(NppXml::saveFileShortcut(_pXmlShortcutDoc, _shortcutsPath.c_str()));
}

void NppParameters::saveMacrosToXmlTree()
{
    if (!_pXmlShortcutDoc)
        return;
    ...
    // Remove old Macros node
    NppXml::Element oldMacros = NppXml::firstChildElement(root, "Macros");
    if (oldMacros)
        NppXml::deleteChild(root, oldMacros);

    // Create new Macros node
    NppXml::Element macrosRoot = NppXml::createChildElement(root, "Macros");

    for (const QPointer<MacroShortcut>& macroSc : std::as_const(_macros)) {
        if (macroSc.isNull())
            continue;
        const Macro& macro = macroSc->getMacro();
        const QString& name = macroSc->getMenuName();
        const KeyCombo& key = macroSc->getKeyCombo();

        NppXml::Element macroRoot = NppXml::createChildElement(macrosRoot, "Macro");
        NppXml::setAttribute(macroRoot, "name", name);
        NppXml::setAttribute(macroRoot, "Ctrl", key._isCtrl ? "yes" : "no");
        NppXml::setAttribute(macroRoot, "Alt", key._isAlt ? "yes" : "no");
        NppXml::setAttribute(macroRoot, "Shift", key._isShift ? "yes" : "no");
        NppXml::setAttribute(macroRoot, "Key", QString::number(key._key));

        for (const RecordedMacroStep& step : macro) {
            NppXml::Element actionNode = NppXml::createChildElement(macroRoot, "Action");
            NppXml::setAttribute(actionNode, "type", QString::number(static_cast<int>(step._macroType)));
            NppXml::setAttribute(actionNode, "message", QString::number(step._message));
            NppXml::setAttribute(actionNode, "wParam", QString::number(static_cast<unsigned long long>(step._wParameter)));
...
{
    // fgColor: sentinel 0xFF000000 means "use default" — skip writing in that case
    if (qAlpha(style2Write._fgColor) != 0 && qRed(style2Write._fgColor) != 0xFF) {
        char fgStr[7];
        std::snprintf(fgStr, sizeof(fgStr), "%.6X", style2Write._fgColor & 0xFFFFFF);
        NppXml::setAttribute(element, "fgColor", fgStr);
    }

    // bgColor
    if (qAlpha(style2Write._bgColor) != 0 && qRed(style2Write._bgColor) != 0xFF) {
        char bgStr[7];
        std::snprintf(bgStr, sizeof(bgStr), "%.6X", style2Write._bgColor & 0xFFFFFF);
    ...
    if (!style2Write._fontName.empty()) {
        const QString newFont = QString::fromWCharArray(style2Write._fontName.c_str());
        const QString oldFont = NppXml::attribute(element, "fontName");
        if (oldFont != newFont) {
            NppXml::setAttribute(element, "fontName", newFont);
            style2Sync._fontName = style2Write._fontName;
        }
    }

    // fontSize
    if (style2Write._fontSize > 0 && style2Write._fontSize != STYLE_NOT_USED) {
        NppXml::setAttribute(element, "fontSize", style2Write._fontSize);
    } else if (style2Write._fontSize == 0) {
        NppXml::setAttribute(element, "fontSize", "");
    }

    // fontStyle: 1=bold, 2=italic, 4=underline (can be combined)
    if (style2Write._fontStyle != STYLE_NOT_USED) {
        NppXml::setAttribute(element, "fontStyle", style2Write._fontStyle);
    }

    // keywords — stored as text content of the element
    QString keywords;
    if (!style2Write._keywords.empty()) {
        keywords = QString::fromStdWString(style2Write._keywords);
    }

    // Set text content (keywords)
    // Try to find existing text node, otherwise create one
    NppXml::Node lastChild = NppXml::lastChild(element);
    if (!lastChild.isNull()) {
        NppXml::setValue(lastChild, keywords.toStdWString());
    } else {
        NppXml::createChildText(element, keywords.toStdWString());
    }
}

std::wstring NppParameters::writeStyles(LexerStylerArray& lexersStylers, StyleArray& globalStylers)
{
    if (!_pXmlUserStylerDoc._doc)
        return {};
    ...
    // Write per-lexer styles: find the LexerType node by name, update its WordsStyle children
    NppXml::Element lexersRoot = NppXml::firstChildElement(root, "LexerStyles");
    for (NppXml::Element lexerNode = NppXml::firstChildElement(lexersRoot, "LexerType");
         !lexerNode.isNull();
         lexerNode = NppXml::nextSiblingElement(lexerNode, "LexerType")) {

        const QString lexerName = NppXml::attribute(lexerNode, "name");
        if (lexerName.isEmpty())
            continue;

        // Update extension
        const LexerStyler* pLs = _lexerStylerVect.getLexerStylerByName(lexerName.toStdWString().c_str());
        const LexerStyler* pLs2 = lexersStylers.getLexerStylerByName(lexerName.toStdWString().c_str());
        if (pLs) {
            NppXml::setAttribute(lexerNode, "ext",
                QString::fromWCharArray(pLs->getLexerUserExt()));
        }

        // Update WordsStyle children
        for (NppXml::Element styleNode = NppXml::firstChildElement(lexerNode, "WordsStyle");
             !styleNode.isNull();
             styleNode = NppXml::nextSiblingElement(styleNode, "WordsStyle")) {

            const QString styleName = NppXml::attribute(styleNode, "name");
            if (styleName.isEmpty())
                continue;

            const Style* pStyle = pLs ? pLs->findByName(styleName.toStdWString()) : nullptr;
            Style* pStyle2Sync = pLs2 ? pLs2->findByName(styleName.toStdWString()) : nullptr;
            if (pStyle && pStyle2Sync)
                writeStyle2Element(*pStyle, *pStyle2Sync, styleNode);
        }
    }

    // Write global styles
    NppXml::Element globalRoot = NppXml::firstChildElement(root, "GlobalStyles");
    if (!globalRoot.isNull()) {
        for (NppXml::Element styleNode = NppXml::firstChildElement(globalRoot, "WidgetStyle");
             !styleNode.isNull();
             styleNode = NppXml::nextSiblingElement(styleNode, "WidgetStyle")) {

            const QString styleName = NppXml::attribute(styleNode, "name");
            if (styleName.isEmpty())
                continue;
            const Style* pStyle = _widgetStyleArray.findByName(styleName.toStdWString());
            Style* pStyle2Sync = globalStylers.findByName(styleName.toStdWString());
            if (pStyle && pStyle2Sync)
                writeStyle2Element(*pStyle, *pStyle2Sync, styleNode);
        }
    }
    ...
    // Save to file
    if (!_pXmlUserStylerDoc._path.empty())
...
// ============================================================================
// Language name → LangType lookup
// Mirrors Win32 NppParameters::getLangIDFromStr()
// Used by main.cpp at startup for -lLangName command-line argument
// ============================================================================
LangType NppParameters::getLangIDFromStr(const wchar_t* langName)
{
	if (!langName || langName[0] == L'\0')
		return L_TEXT;

	for (int lang = 0; lang < static_cast<int>(L_EXTERNAL); ++lang) {
		const char* name = _langNameInfoArray[lang]._langName;
		if (name && std::wcscmp(langName, QString::fromUtf8(name).toStdWString().c_str()) == 0)
...
// ============================================================================
// Localization code → XML filename
// Mirrors Win32 NppParameters::getLocPathFromStr()
// Called from main.cpp / winmain.cpp when -lLocaleCode is passed
// Returns just the filename (e.g. L"french.xml") — caller appends to
// localizationPath to form the full path.
// ============================================================================
std::wstring NppParameters::getLocPathFromStr(const std::wstring& localizationCode)
{
	if (localizationCode.empty())
		return L"";

#define LOC(code, file) if (localizationCode == L ## code) return L ## file;
	LOC("en",     "english.xml")
	LOC("en-au",  "english.xml")
	LOC("en-bz",  "english.xml")
	LOC("en-ca",  "english.xml")
	LOC("en-cb",  "english.xml")
	LOC("en-gb",  "english.xml")
	LOC("en-ie",  "english.xml")
	LOC("en-jm",  "english.xml")
	LOC("en-nz",  "english.xml")
	LOC("en-ph",  "english.xml")
	LOC("en-tt",  "english.xml")
	LOC("en-us",  "english.xml")
	LOC("en-za",  "english.xml")
	LOC("en-zw",  "english.xml")
	LOC("af",     "afrikaans.xml")
	LOC("sq",     "albanian.xml")
	LOC("an",     "aragonese.xml")
	LOC("az",     "azerbaijani.xml")
	LOC("eu",     "basque.xml")
	LOC("be",     "belarusian.xml")
	LOC("bn",     "bengali.xml")
	LOC("bs",     "bosnian.xml")
	LOC("pt-br",  "brazilian_portuguese.xml")
	LOC("bg",     "bulgarian.xml")
	LOC("ca",     "catalan.xml")
	LOC("zh-tw",  "chinese.xml")
	LOC("zh-cn",  "chinese_simplified.xml")
	LOC("zh-sg",  "chinese_simplified.xml")
	LOC("zh-hk",  "chinese_traditional.xml")
	LOC("hr",     "croatian.xml")
	LOC("cs",     "czech.xml")
	LOC("da",     "danish.xml")
	LOC("nl",     "dutch.xml")
	LOC("eo",     "esperanto.xml")
	LOC("et",     "estonian.xml")
	LOC("fi",     "finnish.xml")
	LOC("fr",     "french.xml")
	LOC("gl",     "galician.xml")
	...
	LOC("he",     "hebrew.xml")
	LOC("hi",     "hindi.xml")
	LOC("hu",     "hungarian.xml")
	LOC("id",     "indonesian.xml")
	LOC("it",     "italian.xml")
	LOC("ja",     "japanese.xml")
	LOC("kk",     "kazakh.xml")
	LOC("ko",     "korean.xml")
	LOC("ku",     "kurdish.xml")
	LOC("lv",     "latvian.xml")
	LOC("lt",     "lithuanian.xml")
	LOC("mk",     "macedonian.xml")
	LOC("ms",     "malay.xml")
	LOC("mr",     "marathi.xml")
	LOC("nb",     "norwegian.xml")
	LOC("nn",     "norwegian.xml")
	LOC("no",     "norwegian.xml")
	LOC("pl",     "polish.xml")
	LOC("pt",     "portuguese.xml")
	LOC("ro",     "romanian.xml")
	LOC("ru",     "russian.xml")
	LOC("sr",     "serbian.xml")
	LOC("si",     "sinhala.xml")
	LOC("sk",     "slovak.xml")
	LOC("sl",     "slovenian.xml")
	LOC("es",     "spanish.xml")
	LOC("es-ar",  "spanish.xml")
	LOC("es-bo",  "spanish.xml")
	LOC("es-cl",  "spanish.xml")
	LOC("es-co",  "spanish.xml")
	LOC("es-cr",  "spanish.xml")
	LOC("es-do",  "spanish.xml")
	LOC("es-ec",  "spanish.xml")
	LOC("es-sv",  "spanish.xml")
	LOC("es-gt",  "spanish.xml")
	LOC("es-hn",  "spanish.xml")
	LOC("es-mx",  "spanish.xml")
	LOC("es-ni",  "spanish.xml")
	LOC("es-pa",  "spanish.xml")
	LOC("es-py",  "spanish.xml")
	LOC("es-pe",  "spanish.xml")
	LOC("es-pr",  "spanish.xml")
	LOC("es-uy",  "spanish.xml")
	LOC("es-ve",  "spanish.xml")
	LOC("sv",     "swedish.xml")
	LOC("ta",     "tamil.xml")
	LOC("th",     "thai.xml")
	LOC("tr",     "turkish.xml")
	LOC("uk",     "ukrainian.xml")
	LOC("ur",     "urdu.xml")
	LOC("vi",     "vietnamese.xml")
#undef LOC
	return L"";
}
...
// Mirrors Win32 NppParameters::getLangFromExt()
// Called by Notepad_plus.cpp on every file open for language detection.
// Priority: user lexer extensions → built-in language extensions → L_TEXT
// ============================================================================
LangType NppParameters::getLangFromExt(const wchar_t* ext)
{
	...
	QString extStr = QString::fromWCharArray(ext);
	if (extStr.startsWith('.'))
		extStr = extStr.mid(1);
	extStr = extStr.toLower();

	// 1. Check user-defined lexer extensions from styler list
	//    (already loaded into _lexerStylerVect by feedStylerArray)
	for (size_t i = 0; i < _lexerStylerVect.getNbLexer(); ++i) {
		const LexerStyler& styler = _lexerStylerVect.getLexerFromIndex(i);
		QString extList = QString::fromWCharArray(styler.getLexerUserExt());
		if (extList.isEmpty())
			continue;
		// extList is dot-separated, e.g. "py;pyw"
		for (const QString& e : extList.split(';', Qt::SkipEmptyParts)) {
			if (e.toLower() == extStr) {
				// Found user lexer — get the LangType from lexer name
				return getLangIDFromStr(
					QString::fromUtf8(styler.getLexerName()).toStdWString().c_str()
				);
			}
		}
	}

	// 2. Check built-in language extensions
	//    (from _langList which is populated by feedLang())
	for (size_t i = 0; i < _nbLang; ++i) {
		Lang* lang = getLangFromIndex(i);
		if (!lang || lang->_pExt == nullptr)
			continue;
		for (int eIdx = 0; eIdx < lang->_nbExt; ++eIdx) {
			const wchar_t* bext = lang->_pExt[eIdx];
			if (!bext || bext[0] == L'\0')
				continue;
			QString bextStr = QString::fromWCharArray(bext);
			if (bextStr.startsWith('.'))
				bextStr = bextStr.mid(1);
			if (bextStr.toLower() == extStr)
				return lang->_langID;
		}
	}
...
// ============================================================================
// Language type → default extension string
// Mirrors Win32 NppParameters::getLangExtFromLangType()
// Used by Notepad_plus.cpp for "Save As" and language menu extension display.
// Returns the primary extension without leading dot (e.g. "cpp", "py").
// ============================================================================
const wchar_t* NppParameters::getLangExtFromLangType(LangType langType) const
{
	if (langType < L_TEXT || langType >= L_EXTERNAL)
		return L"";

	// Walk _langList to find matching langID
	for (size_t i = 0; i < _nbLang; ++i) {
		Lang* lang = const_cast<NppParameters*>(this)->getLangFromIndex(i);
		if (!lang || !lang->_pExt)
			continue;
		if (lang->_langID == langType) {
			// Return first non-empty extension
			for (int eIdx = 0; eIdx < lang->_nbExt; ++eIdx) {
				if (lang->_pExt[eIdx] && lang->_pExt[eIdx][0] != L'\0')
					return lang->_pExt[eIdx];
			}
			return L"";
		}
	}
...
// ============================================================================
// Static helper: parse shortcut key info from a <Key> XML element
// Used by feedPluginCustomizedCmds and other feed* functions.
// ============================================================================
bool NppParameters::getShortcuts(const NppXml::Element& element, Shortcut& sc, std::string* /*folderName*/)
{
	...
	const QString name = NppXml::attribute(element, "name");
	const bool isCtrl  = NppXml::intAttribute(element, "Ctrl", 0) != 0;
	const bool isAlt   = NppXml::intAttribute(element, "Alt",  0) != 0;
	const bool isShift = NppXml::intAttribute(element, "Shift", 0) != 0;
	const int key = NppXml::intAttribute(element, "Key", -1);
	if (key == -1)
		return false;

	sc = Shortcut(name, isCtrl, isAlt, isShift, static_cast<quint32>(key));
	return true;
}

// ============================================================================
// Parse <PluginCommands> section from shortcuts.xml
// For each <PluginCommand>, find the matching entry in _pluginCommands
// and apply the shortcut override from the XML.
// ============================================================================
void NppParameters::feedPluginCustomizedCmds(const NppXml::Element& element)
{
	...
	{
		const QString moduleName = NppXml::attribute(childNode, "moduleName");
		...
		// Find the corresponding plugin command and apply the shortcut override
		for (size_t i = 0; i < _pluginCommands.size(); ++i)
		{
			PluginCmdShortcut& pscOrig = _pluginCommands[i];
			if (pscOrig.getModuleName().compare(moduleName, Qt::CaseInsensitive) == 0
				&& pscOrig.getInternalID() == internalID)
			{
				// Found matching command — parse and apply the shortcut
...
// ============================================================================
// Load plugin command shortcuts from shortcuts.xml
// Mirrors Win32 NppParameters::getPluginCmdsFromXmlTree()
// Called by reloadPluginCmds() (in turn triggered by Notepad_plus.cpp:768)
// ============================================================================
...
// ============================================================================
// Save FileBrowser state to user settings XML
// Mirrors Win32 NppParameters::writeFileBrowserSettings()
// Called by Notepad_plus::saveSettings() when file browser is open.
// Writes root folder paths + last-selected item to <FileBrowser> element.
// ============================================================================
bool NppParameters::writeFileBrowserSettings(const std::vector<std::wstring>& rootPaths,
                                             const std::wstring& latestSelectedItemPath)
{
	...
	// Remove old <FileBrowser> node
	NppXml::Element oldFBRoot = NppXml::firstChildElement(nppRoot, "FileBrowser");
	if (oldFBRoot)
		NppXml::deleteChild(nppRoot, oldFBRoot);

	// Create new <FileBrowser> node
	NppXml::Element fbRoot = NppXml::createChildElement(nppRoot, "FileBrowser");

	if (!rootPaths.empty()) {
		NppXml::setAttribute(fbRoot, "latestSelectedItem",
			wstring2string(latestSelectedItemPath));

		for (const auto& rootPath : rootPaths) {
			NppXml::Element fbChild = NppXml::createChildElement(fbRoot, "root");
			NppXml::setAttribute(fbChild, "foldername", wstring2string(rootPath));
		}
	}

	// Save the user doc immediately
	if (_xmlUserDoc._doc && !_xmlUserDoc._path.empty())
		static_cast<void>(NppXml::saveFile(_xmlUserDoc._doc, _xmlUserDoc._path.c_str()));
