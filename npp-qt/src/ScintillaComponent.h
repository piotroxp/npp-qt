...
#include "ScintillaComponent/LexerTypeScript.h"
#include "ScintillaComponent/LexerRust.h"
#include "ScintillaComponent/LexerGo.h"
#include "ScintillaComponent/LexerPowerShell.h"
#include "ScintillaComponent/LexerDockerfile.h"
#include "ScintillaComponent/LexerR.h"
#include "ScintillaComponent/LexerJulia.h"
#include "ScintillaComponent/LexerNim.h"
#include "ScintillaComponent/LexerZig.h"
#include "ScintillaComponent/LexerGraphQL.h"
#include "ScintillaComponent/LexerHaskell.h"
#include "Lexilla.h"
#include "ILexer.h"
#include <vector>
...
// Sci_CharacterRangeFull is defined by NppSciCompat.h (via Scintilla.h).
// Do NOT define it here to avoid redefinition conflicts.
...
// MARK_HIDELINESBEGIN/END defined in NppConstants.h
...
public:
    ...
    intptr_t searchInTarget(const std::string& text, intptr_t& fromPos, size_t toPos) const;
    ...
    intptr_t replaceTarget(const std::string& replacement, intptr_t fromPos = -1, intptr_t toPos = -1);
    ...
    void bookmarkToggle(size_t line) {
        // Toggle bookmark marker: add if absent, delete if present
        // Check marker state via SCI_MARKERGET, then add or delete
        int existing = static_cast<int>(send(SCI_MARKERGET, static_cast<intptr_t>(line)));
        int mask = 1 << MARK_BOOKMARK;
        if (existing & mask) {
            send(SCI_MARKERDELETE, static_cast<intptr_t>(line), mask);
        } else {
            send(SCI_MARKERADD, static_cast<intptr_t>(line), MARK_BOOKMARK);
        }
    }
    ...
    // Set an external Lexilla lexer (ILexer5*) for L_EXTERNAL+N language types
    void setExternalLexer(int externalLangIndex);
...
public:
    ...
    // Set colour for a Scintilla element (SCI_SETELEMENTCOLOUR)
    void setElementColour(int sciElement, QRgb color) const
    {
        send(npp_sci::SCI_SETELEMENTCOLOUR, sciElement, color | 0xFF000000);
    }
...
public:
    // Call tip management — implemented on ScintillaComponent (QsciScintilla)
    // These are stubs on ScintillaCtrls since call tips target a specific editor.
    // Call tip is shown via ScintillaComponent::callTipShow (inherited from QsciScintilla)
    void showCallTip(intptr_t, const std::string&) { /* no-op: call on ScintillaComponent */ }
    void cancelCallTip() { /* no-op: call on ScintillaComponent */ }
