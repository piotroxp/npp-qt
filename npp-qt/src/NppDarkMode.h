...
public:
    ...
    static bool isWindowsModeEnabled_Static() { return instance().isWindowsModeEnabled(); }
    ...
    // setDarkMode(bool) — primary dark-mode switcher (applies Fusion palette + scrollbar style)
    static void setDarkMode(bool enable);
    // Scintilla colour helpers (apply dark fg/bg to all open editor views)
    static void setScintillaFontColor(QsciScintilla* sci, QRgb fgColor);
    static void setScintillaBgColor(QsciScintilla* sci, QRgb bgColor);
...
public:
    ...
    void setDarkTitleBar(QWidget*);
    void setDarkTitleBar(QWidget* w, bool dark);
    ...
    void autoThemeChildControls(QWidget* w);
...
// ── Theme query helpers ────────────────────────────────────────────────────────
// isThemeDark() — true when dark mode is active (singleton enabled flag).
//   Used by ScintillaComponent/AutoCompletion.cpp.
inline bool isThemeDark() { return instance().isEnabled(); }

// ── System dark mode detection (Linux/Qt6) ───────────────────────────────────
// Detects whether the OS / desktop environment is in dark mode.
// On Linux: checks GTK theme dark preference via XDG theme vars,
// falls back to palette luminance check.
// Returns true when the system prefers dark appearance.
inline bool isSystemDark()
{
    // Method 1: Check XDG_CURRENT_DESKTOP and theme variables
    // Many Linux DEs set this environment variable
    static bool checked = false;
    static bool dark = false;
    if (!checked) {
        checked = true;
        const char* xdg = qgetenv("XDG_CURRENT_DESKTOP");
        if (xdg) {
            QString desktop = QString::fromLatin1(xdg).toLower();
            // Check dark mode env vars set by GNOME, KDE, XFCE, etc.
            const char* gtkDark = qgetenv("GTK_THEME");
            if (gtkDark) {
                QString theme = QString::fromLatin1(gtkDark).toLower();
                if (theme.contains("dark")) {
                    dark = true;
                }
            }
            // GNOME dark mode setting
            const char* gnomeDark = qgetenv("GNOME_COLOR_SCHEME");
            if (gnomeDark && QString::fromLatin1(gnomeDark).contains("dark")) {
                dark = true;
            }
        }
        // Fallback: palette luminance check
        if (!dark) {
            dark = qApp->palette().window().color().lightnessF() < 0.5;
        }
    }
    return dark;
}
