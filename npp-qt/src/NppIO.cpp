...
void Notepad_plus::monitorFileOnChange(void * params)
{
	...
	QString abs = d.absolutePath();
	abs.toWCharArray(folderToMonitor);
	folderToMonitor[abs.length()] = 0;
	
	const unsigned int dwNotificationFlags = 0 /* FILE_NOTIFY_CHANGE_* -> QFileSystemWatcher */;
	...
	while (toBeContinued)
	{
	// BLOCKED: WaitForMultipleObjects has no direct Qt6 equivalent.
	// The Qt6 port uses QFileSystemWatcher + signals/slots instead of handles.
	// Refactor to connect CReadDirectoryChanges::filesChanged() signal
	// and QTimer-based monitoring events from Buffer.
	unsigned int waitStatus = 0;  // stub: prevents compile error
		switch (waitStatus)
		{
			case 0 /* WAIT_OBJECT_0+0 */:
			...
			// We've received a notification in the queue.
			{
				...
				while (dirChanges.Pop(dwAction, fn))
				{
					...
					{
						if (dwAction == 1 /* FILE_ACTION_MODIFIED */)
						{
							if (!bDoneOnce)
							{
								// PostMessage(NPPM_INTERNAL_RELOADSCROLLTOEND) -> Qt signal
	QMetaObject::invokeMethod(qobject_cast<QWidget*>(h), "reloadScrollToEnd", Qt::QueuedConnection, Q_ARG(void*, buf));
								bDoneOnce = true;
								QThread::msleep(250);	// Limit refresh rate
							}
						}
						else if ((dwAction == 3 /* FILE_ACTION_REMOVED */) || (dwAction == 4 /* FILE_ACTION_RENAMED_OLD_NAME */))
						{
							// File is deleted or renamed - quit monitoring thread and close file
							// PostMessage(NPPM_INTERNAL_STOPMONITORING) -> Qt signal
	QMetaObject::invokeMethod(qobject_cast<QWidget*>(h), "stopMonitoring", Qt::QueuedConnection, Q_ARG(void*, buf));
						}
					}
				}
			}
...
BufferID Notepad_plus::doOpen(const wstring& fileName, bool isRecursive, bool isReadOnly, int encoding, const wchar_t *backupFileName, NppFileTime fileNameTimestamp)
{
	...
	bool isSnapshotMode = (backupFileName != nullptr) && doesFileExist(backupFileName);
    ...
    if (foundBufID != BUFFER_INVALID && !isSnapshotMode)
    {
        if (_pTrayIco)
        {
            if (_pTrayIco->isInTray())
            {
                ...
                // SendMessage(WM_SIZE) -> QWidget::resize
    ...
    if (buffer != BUFFER_INVALID)
    {
        ...
        _lastRecentFileList.add(QString::fromWCharArray(longFileName));
	...
	if (QWidget* w = qobject_cast<QWidget*>(_pPublicInterface->getHSelf())) w->resize(w->size());
        ...
        _pluginsManager.notify(&scnN);
        // NPPN_UPDATEUI: plugins update toolbar/state after open
...
bool Notepad_plus::doSave(BufferID id, const wchar_t * filename, bool isCopy)
{
	...
	if (!isCopy)
	{
		...
		_pluginsManager.notify(&scnN);
		// NPPN_UPDATEUI: plugins update toolbar/state after save
		scnN.nmhdr.code = NPPN_UPDATEUI;
		_pluginsManager.notify(&scnN);
	}
	...
	std::wstring nppFullPathStr = appPath.toStdWString();
	nppFullPathStr.copy(nppFullPath, MAX_PATH);
	nppFullPath[MAX_PATH-1] = 0;
	...
	if (QWidget* w = qobject_cast<QWidget*>(_pPublicInterface->getHSelf())) { QProcess p; shellExecRes = p.startDetached(QString::fromWCharArray(nppFullPath), QStringList() << QString::fromWCharArray(args.c_str())) ? 33 : 0; }
						...
						{
							if (QWidget* w = qobject_cast<QWidget*>(_pPublicInterface->getHSelf())) w->close();
						}
...
void Notepad_plus::doClose(BufferID id, int whichOne, bool doDeleteBackup)
{
	...
	{	//close the view if both visible
		...
		hiddenBufferID = nullptr /* SendMessage(NPPM_GETBUFFERIDFROMPOS) -> Qt slot */
...
int Notepad_plus::setFileOpenSaveDlgFilters(CustomFileDialog & fDlg, bool showAllExt, int langType)
{
	...
	while (l)
	{
		...
		if (!inExcludedList)
		{
			...
			const wchar_t *userList = nullptr;
...
bool Notepad_plus::fileSave(BufferID bufferID)
{
	...
	if (buf->isDirty())
	{
		...
		{
			const wchar_t *fn = buf->getFullPathName();
			QString nameStr = QFileInfo(QString::fromStdWString(fn)).fileName();
			wstring fn_bak;
...
bool Notepad_plus::fileLoadSession(const wchar_t *fn)
{
	...
	if (QDesktopServices::openUrl(QUrl::fromLocalFile(QString::fromWCharArray(nppFullPath))))
				result = true;
		}
		else
		{
...
{
	// SendMessage(NPPM_INTERNAL_SAVECURRENTSESSION) -> Qt signal
	if (QWidget* w = qobject_cast<QWidget*>(_pPublicInterface->getHSelf()))
		QMetaObject::invokeMethod(w, "saveCurrentSession", Qt::QueuedConnection);
}


// ============================================================================
// FileWatchThread — Qt6 file monitoring using QFileSystemWatcher
// Replaces Win32 monitorFileOnChange() thread + CReadDirectoryChanges
// ============================================================================

FileWatchThread::FileWatchThread(const QString& filePath, QObject* parent)
    : QThread(parent), _filePath(filePath), _stopRequested(false), _loop(nullptr)
{
}
...
void FileWatchThread::run()
{
    ...
    // Validate the file exists before watching
    if (!QFileInfo(_filePath).exists()) {
        qWarning() << "FileWatchThread: file does not exist:" << _filePath;
        return;
    }

    // Add path to watcher
    QStringList paths = watcher.addPath(_filePath);
    if (paths.isEmpty()) {
        qWarning() << "FileWatchThread: failed to watch:" << _filePath;
        return;
    }

    // Forward fileChanged notifications from the watcher to our signal.
    // QFileSystemWatcher emits signals in the thread that created it (this thread),
    // so we must use Qt::QueuedConnection or direct connections here.
    // Nothrow lambda captures 'this' safely since the lambda is destroyed when
    // the thread exits (after _loop->quit()).
    ...
    connect(&watcher, &QFileSystemWatcher::error,
            this, [this](const QString& path) {
        qWarning() << "FileWatchThread: watcher error for:" << path;
    }, Qt::DirectConnection);

    // Create and run event loop — QFileSystemWatcher requires an event loop
    // in the thread that owns it to receive filesystem events.
    _loop = new QEventLoop();
    connect(this, &FileWatchThread::finished, _loop, &QEventLoop::quit);
    connect(this, &FileWatchThread::fileChanged, _loop, &QEventLoop::quit);
    _loop->exec();
}
...
bool FileWatchThread::waitHandle() const
{
    // Replaces Win32 GetWaitHandle() — no equivalent in Qt6.
    // Return true to indicate the watch is active.
    return !_stopRequested.load(std::memory_order_relaxed);
}


// ============================================================================
// NppIO — Qt6 file monitoring using QFileSystemWatcher (main-thread variant)
// ============================================================================

NppIO::NppIO(QObject* parent)
    : QObject(parent)
{
    // Wire up QFileSystemWatcher signals
...
NppIO::~NppIO()
{
    // Stop all watches on destruction
    const QStringList watched = _fileWatcher.files();
    if (!watched.isEmpty()) {
        _fileWatcher.removePaths(watched);
    }
}

void NppIO::startMonitoring(const QString& filePath, Buffer* buffer)
{
    if (filePath.isEmpty() || !buffer)
        return;
    ...
    if (!_fileWatcher.addPath(filePath)) {
        qWarning() << "NppIO: failed to add watch for:" << filePath;
        emit monitoringError(filePath, QStringLiteral("Failed to add file watch"));
        return;
    }

    _monitoredBuffers.insert(filePath, buffer);
    qDebug() << "NppIO: started monitoring:" << filePath;
}
...
void NppIO::stopMonitoring(const QString& filePath)
{
    ...
    _monitoredBuffers.remove(filePath);
    qDebug() << "NppIO: stopped monitoring:" << filePath;
}

void NppIO::onFileChanged(const QString& path)
{
    Buffer* buf = _monitoredBuffers.value(path, nullptr);
    if (!buf) {
        qDebug() << "NppIO::onFileChanged: no buffer for:" << path;
        return;
    }

    // Emit the public signal so connected slots can handle the reload prompt
    emit fileChanged(path);
    qDebug() << "NppIO: file changed externally:" << path;
}

void NppIO::onFileWatchError(const QString& path)
{
    qWarning() << "NppIO: watcher error for:" << path;
    emit monitoringError(path, QStringLiteral("File watcher error"));

    // Attempt to re-add the path — the watcher may recover automatically
    // on some platforms when the file is recreated.
    if (!QFileInfo(path).exists()) {
        qDebug() << "NppIO: file does not exist, removing watch:" << path;
        _fileWatcher.removePath(path);
        _monitoredBuffers.remove(path);
    }
}
