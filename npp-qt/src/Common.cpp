...
// Forward declaration for overloads used in Common.h inline functions
std::wstring string2wstring(const std::string& s, int codepage);
QString string2wstring(const QString& rString, int codepage);

// std::string → std::wstring (converts via QString as Qt6 bridge)
std::wstring string2wstring(const std::string& s, int codepage)
{
    return QString::fromLatin1(s.c_str()).toStdWString();
}
...
bool buf2Clipboard(const QVector<Buffer*>& buffers, bool isFullPath, QWidget* /*parent*/)
{
    ...
    for (const Buffer* buf : buffers)
    {
        if (buf)
        {
            QString name;
            if (isFullPath) {
                name = QString::fromWCharArray(buf->getFullPathName());
            } else {
                name = QString::fromWCharArray(buf->getFileName());
            }
            if (!name.isEmpty()) {
                selection += name + crlf;
            }
