...
// Undef conflicting Scintilla.h INDICATOR_* macros before re-declaring as constexpr.
// When NppConstants.h is included after Scintilla.h in the same TU (e.g. via
// PluginInterface.h -> Scintilla.h -> NppConstants.h), the Scintilla.h INDICATOR_*
// macros would cause syntax errors in the inline constexpr definitions below.
#undef INDICATOR_CONTAINER
#undef INDICATOR_IME
#undef INDICATOR_IME_MAX
#undef INDICATOR_MAX
#undef INDICATOR_HISTORY_REVERTED_TO_ORIGIN_INSERTION
#undef INDICATOR_HISTORY_REVERTED_TO_ORIGIN_DELETION
#undef INDICATOR_HISTORY_SAVED_INSERTION
#undef INDICATOR_HISTORY_SAVED_DELETION
#undef INDICATOR_HISTORY_MODIFIED_INSERTION
#undef INDICATOR_HISTORY_MODIFIED_DELETION
#undef INDICATOR_HISTORY_REVERTED_TO_MODIFIED_INSERTION
#undef INDICATOR_HISTORY_REVERTED_TO_MODIFIED_DELETION
...
inline constexpr int MARK_HIDELINESBEGIN = 19;  // Used by restoreHiddenLines()
inline constexpr int MARK_HIDELINESEND = 18;    // Used by restoreHiddenLines()
...
// NPPN_* notification codes (matches Win32 Notepad_plus_msgs.h ordering)
...
inline constexpr int NPPN_FILEOPENED = NPPN_FIRST + 4;
inline constexpr int NPPN_FILECLOSED = NPPN_FIRST + 5;
inline constexpr int NPPN_FILEBEFOREOPEN = NPPN_FIRST + 6;
inline constexpr int NPPN_FILEBEFORESAVE = NPPN_FIRST + 7;
inline constexpr int NPPN_FILESAVED = NPPN_FIRST + 8;
inline constexpr int NPPN_SHUTDOWN = NPPN_FIRST + 9;  // plugins must stop sending messages after this
inline constexpr int NPPN_BUFFERACTIVATED = NPPN_FIRST + 10;
inline constexpr int NPPN_LANGCHANGED = NPPN_FIRST + 11;
inline constexpr int NPPN_WORDSTYLESUPDATED = NPPN_FIRST + 12;
inline constexpr int NPPN_SHORTCUTREMAPPED = NPPN_FIRST + 13;
inline constexpr int NPPN_READCONFIGS = NPPN_FIRST + 14;
inline constexpr int NPPN_AREYOUSEARCHING = NPPN_FIRST + 15;
inline constexpr int NPPN_DIRLISTING = NPPN_FIRST + 16;
inline constexpr int NPPN_SESSIONLOADED = NPPN_FIRST + 17;
inline constexpr int NPPN_SESSIONCLOSED = NPPN_FIRST + 18;
inline constexpr int NPPN_SESSIONSAVE = NPPN_FIRST + 19;
inline constexpr int NPPN_DARKMODECHANGED = NPPN_FIRST + 20;
inline constexpr int NPPN_SOFTRECALC = NPPN_FIRST + 21;
inline constexpr int NPPN_DOCMODIFIED = NPPN_FIRST + 22;
inline constexpr int NPPN_UPDATEUI = NPPN_FIRST + 23;
inline constexpr int NPPN_CURRENTSITZCHANGED = NPPN_FIRST + 24;
inline constexpr int NPPN_DOCORDERCHANGED = NPPN_FIRST + 25;
inline constexpr int NPPN_SNAPSHOTDIRTYFILELOADED = NPPN_FIRST + 26;
inline constexpr int NPPN_QUERYMODIFIED = NPPN_FIRST + 27;
inline constexpr int NPPN_CANCEL = NPPN_FIRST + 28;
inline constexpr int NPPN_FILEDELETED = NPPN_FIRST + 29;
...
enum LangType {
    ...
    L_CMAKE = 48,      L_YAML = 49,    L_DOCKERFILE = 97, L_COBOL = 50,     L_GUI4CLI = 51,
    ...
    L_MARKDOWN = 96,
    L_JULIA = 98,   L_ZIG = 99,   L_GRAPHQL = 100,
};
...
struct FindersInfo {
    ...
    struct FindOption {
        ...
        std::wstring _directory;   // directory path for Find in Files
    } _findOption;
    QString _findRes;
    int _nbMatch = 0;
};
...
struct RECT {
    ...
    long right;
    long bottom;
    // Named rectWidth/rectHeight to avoid ambiguity with QRect::width()/height()
    long rectWidth()  const { return right - left; }
    long rectHeight() const { return bottom - top; }
    // Explicit conversion to QRect (avoid Q_DECL_CONSTEXPR to prevent GCC constexpr issues)
    operator QRect() const { return QRect(left, top, rectWidth(), rectHeight()); }
    QRect toQRect() const { return QRect(left, top, rectWidth(), rectHeight()); }
};
#endif
#ifndef LPRECT
using LPRECT = RECT*;
