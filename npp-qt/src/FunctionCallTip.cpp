...
bool FunctionCallTip::updateCalltip(int ch, bool needShown)
{
    if (!needShown && ch != _start && ch != _param && !isVisible())
        return false;
    _curPos = _pEditView ? _pEditView->send(npp_sci::SCI_GETCURRENTPOS) : 0;
...
{
    return _pEditView && _pEditView->send(npp_sci::SCI_CALLTIPACTIVE) != 0;
}

void FunctionCallTip::close()
{
    if (!isVisible() || !_selfActivated) return;
    if (_pEditView) _pEditView->send(npp_sci::SCI_CALLTIPCANCEL);
...
bool FunctionCallTip::getCursorFunction()
{
    ...
    auto line = _pEditView->send(npp_sci::SCI_LINEFROMPOSITION, _curPos);
    auto startpos = _pEditView->send(npp_sci::SCI_POSITIONFROMLINE, line);
    auto endpos = _pEditView->send(npp_sci::SCI_GETLINEENDPOSITION, line);
    ...
    _pEditView->send(npp_sci::SCI_GETLINE, line, reinterpret_cast<sptr_t>(lineBytes.data()));
...
void FunctionCallTip::showCalltip()
{
    ...
    if (isVisible())
        _pEditView->send(npp_sci::SCI_CALLTIPCANCEL);
    else
        _startPos = _curPos;
    _pEditView->send(npp_sci::SCI_CALLTIPSHOW, _startPos, reinterpret_cast<sptr_t>(tipStr.toUtf8().constData()));
    _selfActivated = true;
}
