...
vector<pair<intptr_t, intptr_t>> XmlMatchedTagsHighlighter::getAttributesPos(intptr_t start, intptr_t end)
{
    ...
    _pEditView->send(npp_sci::SCI_GETTEXTRANGE, start,
    ...
    const string& tagName, intptr_t start, intptr_t end)
{
    ...
    do {
        ...
        if (r.success) {
            intptr_t nextChar = _pEditView ? _pEditView->send(npp_sci::SCI_GETCHARAT, r.end) : 0;
            styleAt = _pEditView ? _pEditView->send(npp_sci::SCI_GETSTYLEAT, r.start) : 0;
            if (styleAt != 4 && styleAt != 5 && styleAt != 6 && styleAt != 9) { // not in string/comment
                if (nextChar == '>') {
                    ...
                    if (closeAngle != -1 && _pEditView && _pEditView->send(npp_sci::SCI_GETCHARAT, closeAngle - 1) != '/')
...
intptr_t XmlMatchedTagsHighlighter::findCloseAngle(intptr_t startPosition, intptr_t endPosition)
{
    ...
    while (searchStart < endPosition) {
        ...
        if (!r.success) break;
        intptr_t style = _pEditView->send(npp_sci::SCI_GETSTYLEAT, r.start);
    ...
    const string& tagName, intptr_t start, intptr_t end)
{
    ...
    do {
        ...
        if (!r.success) break;
        intptr_t nextChar = _pEditView ? _pEditView->send(npp_sci::SCI_GETCHARAT, r.end) : 0;
        intptr_t styleAt = _pEditView ? _pEditView->send(npp_sci::SCI_GETSTYLEAT, r.start) : 0;
        if (styleAt != 4 && styleAt != 5 && styleAt != 6 && styleAt != 9) {
            if (nextChar == '>') {
                result = r; result.success = true; result.end = r.end + 1; break;
            } else if (isWhitespace(nextChar)) {
                intptr_t wp = r.end;
                while (_pEditView && wp < end && isWhitespace(_pEditView->send(npp_sci::SCI_GETCHARAT, wp))) wp++;
                if (_pEditView && _pEditView->send(npp_sci::SCI_GETCHARAT, wp) == '>')
    ...
    const char* text, intptr_t start, intptr_t end, int flags)
{
    ...
    _pEditView->send(npp_sci::SCI_SETTARGETSTART, start);
    _pEditView->send(npp_sci::SCI_SETTARGETEND, end);
    intptr_t result = _pEditView->send(npp_sci::SCI_SEARCHINTARGET, strlen(text), reinterpret_cast<sptr_t>(text));
    if (result == -1) return returnValue;
    returnValue.success = true;
    returnValue.start = _pEditView->send(npp_sci::SCI_GETTARGETSTART);
    returnValue.end = _pEditView->send(npp_sci::SCI_GETTARGETEND);
    return returnValue;
}
...
bool XmlMatchedTagsHighlighter::getXmlMatchedTagsPos(XmlMatchedTagsPos& xmlTags)
{
    ...
    bool tagFound = false;
    intptr_t caret = _pEditView->send(npp_sci::SCI_GETCURRENTPOS);
    intptr_t searchStartPoint = caret;
    FindResult openFound = findText("<", searchStartPoint, 0, 0);
    intptr_t styleAt = _pEditView->send(npp_sci::SCI_GETSTYLEAT, openFound.start);
    ...
    while (openFound.success &&
        ...
        styleAt = _pEditView->send(npp_sci::SCI_GETSTYLEAT, openFound.start);
        searchStartPoint = openFound.start - 1;
    }
    if (!openFound.success || styleAt == 4) return false; // in CDATA

    FindResult closeFound = findText(">", openFound.start, caret, 0);
    intptr_t closeStyle = _pEditView->send(npp_sci::SCI_GETSTYLEAT, closeFound.start);
    ...
    while (closeFound.success &&
        ...
        closeStyle = _pEditView->send(npp_sci::SCI_GETSTYLEAT, closeFound.start);
    }

    if (!closeFound.success) {
        intptr_t nextChar = _pEditView->send(npp_sci::SCI_GETCHARAT, openFound.start + 1);
        if (nextChar == '/') {
            // Cursor in close tag
            xmlTags.tagCloseStart = openFound.start;
            intptr_t docLen = _pEditView->send(npp_sci::SCI_GETLENGTH);
            ...
            intptr_t docLen2 = _pEditView->send(npp_sci::SCI_GETLENGTH);
            string tagName;
            nextChar = _pEditView->send(npp_sci::SCI_GETCHARAT, position);
            ...
            if (!tagName.empty()) {
                ...
                if (closeAnglePos != -1) {
                    ...
                    if (_pEditView->send(npp_sci::SCI_GETCHARAT, closeAnglePos - 1) == '/') {
...
void XmlMatchedTagsHighlighter::tagMatch(bool doHiliteAttr)
{
    ...
    if (getXmlMatchedTagsPos(xmlTags)) {
        _pEditView->send(npp_sci::SCI_SETINDICATORCURRENT, 31);
        int openTagTailLen = 2;
        if (xmlTags.tagCloseStart != -1 && xmlTags.tagCloseEnd != -1) {
            _pEditView->send(npp_sci::SCI_INDICATORFILLRANGE, xmlTags.tagCloseStart, xmlTags.tagCloseEnd - xmlTags.tagCloseStart);
            openTagTailLen = 1;
        }
        _pEditView->send(npp_sci::SCI_INDICATORFILLRANGE, xmlTags.tagOpenStart, xmlTags.tagNameEnd - xmlTags.tagOpenStart);
        _pEditView->send(npp_sci::SCI_INDICATORFILLRANGE, xmlTags.tagOpenEnd - openTagTailLen, openTagTailLen);
        ...
        if (doHiliteAttr && xmlTags.tagOpenEnd > xmlTags.tagNameEnd && xmlTags.tagOpenEnd > 0) {
            ...
            _pEditView->send(npp_sci::SCI_SETINDICATORCURRENT, 32);
            for (auto& attr : attrs)
                _pEditView->send(npp_sci::SCI_INDICATORFILLRANGE, attr.first, attr.second - attr.first);
        }

        if (_pEditView->send(npp_sci::SCI_GETINDENTATIONGUIDES) != 0 && xmlTags.tagCloseStart != -1) {
            intptr_t colAtCaret = _pEditView->send(npp_sci::SCI_GETCOLUMN, xmlTags.tagOpenStart);
            intptr_t colOpposite = _pEditView->send(npp_sci::SCI_GETCOLUMN, xmlTags.tagCloseStart);
            intptr_t lineAtCaret = _pEditView->send(npp_sci::SCI_LINEFROMPOSITION, xmlTags.tagOpenStart);
            intptr_t lineOpposite = _pEditView->send(npp_sci::SCI_LINEFROMPOSITION, xmlTags.tagCloseStart);
            if (xmlTags.tagCloseStart != -1 && lineAtCaret != lineOpposite)
                _pEditView->send(npp_sci::SCI_BRACEHIGHLIGHT, xmlTags.tagOpenStart, xmlTags.tagCloseEnd - 1);
        }
    }
}
