...
// Load an XML file from a QString path (Qt-native string)
inline bool loadFile(Document& doc, const QString& filePath) {
    return loadFile(doc, filePath.toStdWString().c_str());
}
...
inline void setAttribute(Element elem, const char* attrName, unsigned int value) {
    if (!elem.isNull()) elem.setAttribute(QLatin1String(attrName), value);
}
