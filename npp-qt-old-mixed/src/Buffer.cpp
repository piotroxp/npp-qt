...
LangType Buffer::langFromExtension(const std::wstring& ext) {
    ...
    if (qext == "m") return LangType::L_OBJC;          // Objective-C .m
    if (qext == "java") return LangType::L_JAVA;
    if (qext == "kt" || qext == "kts") return LangType::L_CPP;  // Kotlin → C++ lexer (no Kotlin lexer)
    if (qext == "cs") return LangType::L_CS;          // C# → C# (uses C++ lexer)
    if (qext == "go") return LangType::L_GOLANG;
    if (qext == "rs") return LangType::L_RUST;
    if (qext == "swift") return LangType::L_SWIFT;
    if (qext == "mjs") return LangType::L_JAVASCRIPT;
    if (qext == "jsx") return LangType::L_JAVASCRIPT;
    if (qext == "ts" || qext == "tsx") return LangType::L_TYPESCRIPT;
    if (qext == "js") return LangType::L_JAVASCRIPT;
    if (qext == "csx") return LangType::L_COFFEESCRIPT;
    // Web languages
    ...
    if (qext == "jsp") return LangType::L_JSP;
    // Scripting languages
    if (qext == "py" || qext == "pyw") return LangType::L_PYTHON;
    if (qext == "sh" || qext == "bash" || qext == "zsh" || qext == "fish" ||
        qext == "ksh" || qext == "csh") return LangType::L_BASH;
    if (qext == "bat" || qext == "cmd" || qext == "nt") return LangType::L_BATCH;
    if (qext == "ps1" || qext == "psm1" || qext == "psd1") return LangType::L_POWERSHELL;
    if (qext == "pl" || qext == "pm" || qext == "t") return LangType::L_PERL;
    if (qext == "rb" || qext == "rake" || qext == "erb") return LangType::L_RUBY;
    if (qext == "lua") return LangType::L_LUA;
    if (qext == "tcl") return LangType::L_TCL;
    if (qext == "r" || qext == "R" || qext == "Rprofile") return LangType::L_R;
    if (qext == "raku") return LangType::L_RAKU;
    if (qext == "nim") return LangType::L_NIM;
    if (qext == "gd") return LangType::L_GDSCRIPT;
    // Data/config formats
    if (qext == "json" || qext == "json5") return LangType::L_JSON;
    if (qext == "yaml" || qext == "yml") return LangType::L_YAML;
    if (qext == "toml") return LangType::L_TOML;
    if (qext == "xml") return LangType::L_XML;
    if (qext == "ini" || qext == "conf" || qext == "cfg") return LangType::L_INI;
    if (qext == "props" || qext == "properties") return LangType::L_PROPS;
    if (qext == "registry") return LangType::L_REGISTRY;
    if (qext == "makefile") return LangType::L_MAKEFILE;
    if (qext == "cmake") return LangType::L_CMAKE;
    // Systems languages
    if (qext == "asm" || qext == "masm" || qext == "nasm" || qext == "asm64") return LangType::L_ASM;
    if (qext == "vhdl" || qext == "vhd") return LangType::L_VHDL;
    if (qext == "v" || qext == "sv") return LangType::L_VERILOG;
    if (qext == "pas" || qext == "pp") return LangType::L_PASCAL;
    if (qext == "f90" || qext == "f95" || qext == "f03") return LangType::L_FORTRAN;
    // Functional/special-purpose
    ...
    if (qext == "tex") return LangType::L_TEX;
    if (qext == "vb" || qext == "vbs") return LangType::L_VB;
    // Documentation
    if (qext == "md" || qext == "markdown" || qext == "mdown" || qext == "mkd") return LangType::L_MARKDOWN;
    // Fallback
    if (qext == "txt") return LangType::L_TEXT;
...
bool FileManager::loadFileData(Document doc, int64_t fileSize, const wchar_t* fn, char* data, Utf8_16_Read* uc, LoadedFileFormat& lf)
{
    ...
    if (lf._language < LangType::L_EXTERNAL) {
        // Use null lexer initially, MainWindow applies the QsciLexer
        _pscratchTilla->send(SCI_SETILEXER, 0, static_cast<sptr_t>(nullptr));
    } else {
        // External lexer from Lexilla plugin — use fnCL to create ILexer5*
        int id = lf._language - LangType::L_EXTERNAL;
        NppParameters& nppParams = NppParameters::getInstance();
        const ExternalLangContainer* externalLexer = nppParams.getELCFromIndex(id);
        if (externalLexer && externalLexer->fnCL) {
            auto CreateLexer = reinterpret_cast<Lexilla::CreateLexerFn>(
                externalLexer->fnCL);
            ILexer5* iLexer = CreateLexer(externalLexer->_name.c_str());
            _pscratchTilla->send(SCI_SETILEXER, 0, reinterpret_cast<sptr_t>(iLexer));
        }
    }
    if (lf._encoding != -1)
...
void Buffer::increaseRecentTag() {
    // Increment the recent tag counter — used by Recent Files list for MRU ordering.
    // Mirrors Win32 MRU FILETIME stamp. Higher tag = more recently used.
