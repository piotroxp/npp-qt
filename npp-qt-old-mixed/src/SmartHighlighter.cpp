...
void SmartHighlighter::highlightView(ScintillaComponent* pHighlightView, ScintillaComponent* unfocusView)
{
    ...
    // Smart highlighting respects NppSettings ("Perf.SmartHighlighting")
    if (!NppSettings::instance().get(QStringLiteral("Perf.SmartHighlighting"), true).toBool())
        return;
    ...
    bool highlightOnAnotherView = NppSettings::instance().get(QStringLiteral("SmartHilite.OnAnotherView"), true).toBool();
    if (highlightOnAnotherView && unfocusView && unfocusView->isVisible())
    {
...
void SmartHighlighter::highlightViewWithWord(ScintillaComponent* pHighlightView, const std::wstring& word2Hilite)
{
    ...
    // Settings wired via NppSettings (mirrors Win32 NppGUI / FindHistory)
    bool isWordOnly = NppSettings::instance().get(QStringLiteral("SmartHilite.WholeWord"), true).toBool();
    bool isCaseSensitive = NppSettings::instance().get(QStringLiteral("SmartHilite.CaseSensitive"), true).toBool();
