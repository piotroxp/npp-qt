...
{
    // Use POSIX access() for speed — avoids QFileInfo slowness on sandboxed filesystems
    return ::access(filePath.toLocal8Bit().constData(), F_OK) == 0;
}
...
bool doesDirectoryExist(const QString& dirPath)
{
    // Use POSIX access() for speed — avoids QFileInfo slowness on sandboxed filesystems
    struct stat st;
    if (::stat(dirPath.toLocal8Bit().constData(), &st) != 0)
        return false;
...
// Build Recent File menu entries: "1: filename", "2: filename", etc.
// filenameLen: max display width (characters); truncates filename if needed
// pos: 0-based index, used to generate ordinal prefix (&1, &2, ...)
// ordinalNumber: if true, prepend "&N: " prefix; if false just prefix "&"
QString buildMenuFileName(int filenameLen, unsigned int pos,
                          const QString& filename, bool ordinalNumber)
{
    QString result;
    ...
    if (ordinalNumber) {
        if (pos < 9) {
            result.append(QChar('&'));
            result.append(QChar('1' + pos));
        } else if (pos == 9) {
            result.append(QStringLiteral("1&0"));
        } else {
            int quot = (pos + 1) / 10;
            int rem  = (pos + 1) % 10;
            result.append(QString::number(quot));
            result.append(QChar('&'));
            result.append(QString::number(rem));
        }
    ...
    if (filenameLen > 0) {
        // Truncate filename to filenameLen chars
        if (filename.length() > filenameLen) {
            result.append(filename.left(filenameLen - 3));
            result.append(QStringLiteral("..."));
        } else {
            result.append(filename);
        }
    } else {
        // filenameLen <= 0: show just the filename component (basename)
        QFileInfo fi(filename);
        ...
        if (filenameLen < 0) {
            // Show full basename, no truncation
            result.append(basename);
        } else {
            // filenameLen == 0: same as negative
            result.append(basename);
        }
    }
