...
{
#ifndef _WIN32
	// Qt/POSIX implementation using QCollator
	QCollator collator;
	if (!localeName.empty())
		collator.setLocale(QLocale(QString::fromStdWString(localeName)));
	collator.setCaseSensitivity(caseSensitive ? Qt::CaseSensitive : Qt::CaseInsensitive);
	collator.setNumericMode(digitsAsNumbers);
	collator.setIgnoreDiacritics(ignoreDiacritics);
	// ignoreSymbols: QCollator doesn't have a direct equivalent; locale-based comparison is used
	...
	switch (sci->execute(SCI_GETSELECTIONMODE))
	{
		...
		case SC_SEL_RECTANGLE:
			...
			if (bottomLine == sci->execute(SCI_GETLINECOUNT) - 1)
			{
		...
		default:
			...
			if (anchor == caret)
			{
				...
				endPos = sci->execute(SCI_GETLENGTH);
				if (sci->execute(SCI_POSITIONFROMLINE, bottomLine) == endPos)
					bottomLine--;
				else
					missingEOL = true;
			}
			else
			{
				...
				else if (bottomLine == sci->execute(SCI_GETLINECOUNT) - 1)
				{
					missingEOL = true;
					endPos = sci->execute(SCI_GETLENGTH);
				}
	...
	try
	{
		...
		for (SortLine& sl : sortLines)
		{
			...
			if (!keyText.empty())
			{
				// Convert to QString for collation
				QString keyStr = QString::fromLatin1(keyText.data(), static_cast<int>(keyText.length()));
				sl.key = collator.sortKey(keyStr).string().toStdString();
			}
		}
	...
	catch (const std::exception& e)
	{
		try
		{
			QString errmsg = QString::fromLocal8Bit(e.what());
			return { MB_ICONERROR, "SortLocaleExcept", errmsg.toStdWString() };
		}
		catch (...)
		{
			return errorUnknown;
	...
	if (rectangular)
	{
		// Rectangular selection restore not implemented in this path
		(void)0;
	}
	else if (noselection)
...
#else
	// Windows implementation using native APIs
	...
	if (replaceStatus != SC_STATUS_OK && replaceStatus < SC_STATUS_WARN_START)
	{
		...
		SendMessage(sci->getHSelf(), WM_SETREDRAW, FALSE, 0);
		if (replaceStatus == SC_STATUS_BADALLOC)
			throw ScintillaMemory();
		else
			throw ScintillaFail();
