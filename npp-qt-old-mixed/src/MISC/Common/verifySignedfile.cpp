...
// verifySignedfile.cpp : Verification of an Authenticode signed DLL
// On non-Windows platforms, this file provides stub implementations.
...
bool SecurityGuard::checkModule([[maybe_unused]] const std::wstring& filePath, [[maybe_unused]] NppModule module2check)
{
	...
	if (_securityMode == sm_certif)
		...
		return false;
#else
	// On non-Windows platforms, code signing verification is not supported
	return false;
#endif
#endif
}
...
bool SecurityGuard::verifySignedBinary(const std::wstring& filepath)
{
	...
	winTEXTrust_data.dwUIChoice = WTD_UI_NONE;
	winTEXTrust_data.dwUnionChoice = WTD_CHOICE_FILE;
	winTEXTrust_data.dwStateAction = WTD_STATEACTION_VERIFY;
	winTEXTrust_data.fdwRevocationChecks = WTD_REVOKE_WHOLECHAIN;
