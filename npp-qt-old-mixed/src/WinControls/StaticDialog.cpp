...
QString RunDlg::expandVariables(const QString& input) {
...
void RunDlg::addToHistory(const QString& text) {
...
void RunDlg::removeFromHistory(const QString& text) {
    _history.removeAll(text);
    _historyCombo->clear();
    _historyCombo->addItems(_history);
}

intptr_t RunDlg::run_dlgProc(unsigned int message, intptr_t wParam, intptr_t lParam) {
    switch (message) {
