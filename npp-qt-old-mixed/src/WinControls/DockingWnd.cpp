...
DockedWidgetData* DockingContainer::addDockedWidget(const DockedWidgetData& data) {
    ...
    _tabWidget->addTab(pData->client, pData->iconTab.isNull() ? QIcon() : pData->iconTab, tabText);
...
void DockingManager::relayout() {
    ...
    _splitters[0]->setVisible(_containers[CONT_LEFT]->hasWidgets() == false &&
                               _containers[CONT_RIGHT]->hasWidgets() == false);
    _splitters[1]->setVisible(_containers[CONT_TOP]->hasWidgets() == false &&
                               _containers[CONT_BOTTOM]->hasWidgets() == false);
}
