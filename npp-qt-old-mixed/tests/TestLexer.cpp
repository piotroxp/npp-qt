...
private slots:
    ...
    void test_langFromExtension_powershell();
    ...
    void test_createLexer_typescript();
...
void TestLexer::test_langFromExtension_powershell()
{
    QBENCHMARK {
        LangType t = Buffer::langFromExtension(L".ps1");
        if (t != LangType::L_POWERSHELL)
            qWarning("ps1 returned %d instead of L_POWERSHELL", int(t));
        LangType t2 = Buffer::langFromExtension(L".psm1");
        if (t2 != LangType::L_POWERSHELL)
            qWarning("psm1 returned %d instead of L_POWERSHELL", int(t2));
    }
}
...
void TestLexer::test_createLexer_typescript()
{
    QsciLexer* lex = _editor->createLexerForLanguage(L_TYPESCRIPT);
    QVERIFY2(lex != nullptr, "TypeScript lexer must not be null");
    delete lex;
}
